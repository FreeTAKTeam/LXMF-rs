...
fn work_comment_missing_content_matches_pinned_python_without_mutation() {
    use std::path::PathBuf;
    use std::process::Command;

    const PYTHON_REFERENCE_REVISION: &str = "99de23c040d507e3fefca19e87b182302902725d";
    const REMOTE: [u8; 16] = [0x63; 16];
    ...
    let root_bytes = rngit_work_fixture(&root);
    fs::write(rust_document.join("root"), &root_bytes).expect("seed Rust root");
    ...
    assert!(seed.success(), "could not seed pinned Python document");
...
request = {0: "group/repo", "operation": "comment", "doc_id": int(doc_id), "scope": "active"}
...
"#;
    ...
    assert_eq!(python_result["body"], "Content is required");
    ...
    let mut rust_files = fs::read_dir(&rust_document)
        .expect("read Rust document files")
        .map(|entry| {
            let entry = entry.expect("Rust document entry");
            (
                entry.file_name().to_string_lossy().into_owned(),
                hex::encode(fs::read(entry.path()).expect("read Rust document file")),
            )
        })
        .collect::<Vec<_>>();
    rust_files.sort();
