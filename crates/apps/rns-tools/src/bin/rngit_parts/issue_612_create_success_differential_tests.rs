...
fn work_create_success_matches_pinned_python_response_and_document_state() {
    ...
    let signer = rns_transport::identity::PrivateIdentity::new_from_name("issue-612-create");
    ...
    .concat();
    let title = "Created document";
    let content = "authenticated active work";
...
response = node.handle_work("/mgmt/work", {
    0: "group/repo", "operation": "create", "title": title,
    "content": content, "format": "markdown", "signature": bytes.fromhex(signature_hex),
}, 1, identity, 0)
work = repo + ".work"
with open(os.path.join(work, "active", "1", "root"), "rb") as stream:
...
print(json.dumps({
    ...
    "meta": {k: (v.hex() if isinstance(v, bytes) else v) for k, v in document["meta"].items()},
}))
"#;
    let python = Command::new(std::env::var_os("LXMF_PYTHON_BIN").unwrap_or_else(|| "python3".into()))
        ...
        .arg(title).arg(content).arg(hex::encode(&signature))
    ...
    let rust_response = rust_node.handle_work_request_with_peer_identity(
        &request, remote, Some(*signer.as_identity()),
    );
    ...
    let rust_root = rust_group.join("repo.work/active/1/root");
    let rust_document = rust_node.work_load_document(&rust_root).expect("Rust active document");
    let rust_map = rust_document.as_map().expect("Rust document map");
    let rust_meta = map_value(rust_map, &rmpv::Value::from("meta")).and_then(rmpv::Value::as_map).expect("Rust metadata");
    assert_eq!(map_value(rust_map, &rmpv::Value::from("content")).and_then(rmpv::Value::as_str), Some(content));
    ...
    for key in ["created", "edited"] {
        let python_timestamp = python["meta"][key].as_f64().expect("Python timestamp");
        let rust_timestamp = map_value(rust_meta, &rmpv::Value::from(key))
            .and_then(rmpv::Value::as_f64)
            .expect("Rust timestamp");
        assert!((python_timestamp - rust_timestamp).abs() <= 1.0, "timestamp delta for {key}");
    }
    assert!(python["active"].as_bool().expect("Python active state"));
    assert!(!python["proposed"].as_bool().expect("Python proposed state"));
    assert!(rust_group.join("repo.work/active/1").is_dir());
    assert!(!rust_group.join("repo.work/proposed/1").exists());
}
