...
fn run_git_fetch(
    ...
    let remote_sha = remote_refs
        ...
        .ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "remote Git reference was not found"))?;

    let response = client
        .process_fetch_queue(remote, &[reference.to_owned()])
        .map_err(io::Error::other)?;
    if response.first().copied() != Some(0) {
        let detail = String::from_utf8_lossy(response.get(1..).unwrap_or_default()).trim().to_owned();
    ...
    let mut bundle = tempfile::Builder::new().prefix("rngit-fetch-").suffix(".bundle").tempfile()?;
