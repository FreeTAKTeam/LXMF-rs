...
fn lxmd_three_node_tcp_full_test_waits_for_announces_before_delivery() {
    let Some(lxmd_bin) = option_env!("CARGO_BIN_EXE_lxmd") else {
        panic!("cargo did not provide CARGO_BIN_EXE_lxmd for integration test")
    };
    let Some(reticulumd_bin) = option_env!("CARGO_BIN_EXE_reticulumd") else {
        panic!("cargo did not provide CARGO_BIN_EXE_reticulumd for integration test")
    };

    let temp = tempfile::tempdir().expect("tempdir");
    let server_port = unique_port(38_000);
    let server_rpc = unique_port(39_000);
    let client_two_rpc = unique_port(40_000);
    let client_three_rpc = unique_port(41_000);
...
fn spawn_lxmd(lxmd_bin: &str, reticulumd_bin: &str, config_dir: &Path) -> Child {
...
fn wait_for_peer(rpc_port: u16, expected_peer: &str) -> Result<(), String> {
    ...
    while Instant::now() < deadline {
        ...
        let seen = peers["peers"].as_array().is_some_and(|items| {
            items.iter().any(|peer| peer["peer"].as_str() == Some(expected_peer))
        });
        if seen {
            return Ok(());
        }
...
fn decode_rpc_frame(bytes: &[u8]) -> Result<Value, String> {
    ...
    serde_json::from_slice(&bytes[4..4 + frame_len]).map_err(|err| err.to_string())
}
...
fn unique_port(base: u16) -> u16 {
    let pid = std::process::id() as u16;
    base.saturating_add(pid % 1000)
}
