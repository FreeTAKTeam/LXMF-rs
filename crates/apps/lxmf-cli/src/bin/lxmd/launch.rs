...
pub(crate) fn launch_supervised(
    ...
    let mut child = match cmd.spawn() {
        Ok(child) => child,
        Err(err) => {
            log::error!("lxmd: failed to launch {}: {}", reticulumd.display(), err);
    ...
    if args.propagation_node {
        if let Err(err) = enable_propagation_mode(rpc_addr) {
            log::error!("lxmd: failed to enable propagation mode: {err}");
            let _ = child.kill();
            let _ = child.wait();
            return ExitCode::from(1);
        }
    ...
    if let Err(err) = apply_python_compat_config(rpc_addr, args) {
        log::error!("lxmd: failed to apply python-style daemon settings: {err}");
        let _ = child.kill();
        let _ = child.wait();
        return ExitCode::from(1);
    }
    ...
    if args.propagation_node {
        if let Err(err) = rpc_client::rpc_call(rpc_addr, "announce_now", None) {
            log::error!("lxmd: failed to announce propagation state: {err}");
        }
    }
    ...
    match child.wait() {
        ...
        Err(err) => {
            log::error!("lxmd: failed waiting for reticulumd: {}", err);
            ExitCode::from(1)
        }
    }
}
