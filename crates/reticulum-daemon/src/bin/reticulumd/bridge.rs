...
async fn deliver_record_task(
    ...
    let Some(identity) = identity else {
        ...
        let _ = receipt_tx
            .send(ReceiptEvent { message_id, status: "failed: peer not announced".to_string() });
        return;
    };
    ...
    match result {
        Ok(LinkSendResult::Packet(packet)) => {
            ...
            let _ = receipt_tx.send(ReceiptEvent { message_id, status: "sent: link".to_string() });
        }
        Ok(LinkSendResult::Resource(resource_hash)) => {
            let detail = format!("resource_hash={}", hex::encode(resource_hash.as_slice()));
            log_delivery_trace(&message_id, &destination_hex, "link", &detail);
            let _ = receipt_tx
                .send(ReceiptEvent { message_id, status: "sending: link resource".to_string() });
        }
        Err(err) => {
...
impl OutboundBridge for TransportBridge {
    fn deliver(
        ...
        tokio::spawn(deliver_record_task(
            transport,
            peer_crypto,
            receipt_map,
            receipt_tx,
