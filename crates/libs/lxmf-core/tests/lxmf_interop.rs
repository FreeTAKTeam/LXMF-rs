...
fn pack_stamp_roundtrip_matches_reference_fixture() {
    ...
    let packed = wire.pack().unwrap();
    let fixture = fixture_bytes("direct_with_stamp");
    assert_eq!(packed, fixture, "{}", first_diff(&packed, &fixture));

    let reparsed = WireMessage::unpack(&fixture).unwrap();
    assert_eq!(reparsed.payload.stamp.as_ref().unwrap().as_ref(), vec![0x44; 32].as_slice());

    let repacked = reparsed.pack().unwrap();
    assert_eq!(repacked, fixture, "{}", first_diff(&repacked, &fixture));
}

#[test]
fn rust_fixture_matches_python_binary_envelope() {
    let packed = deterministic_wire(None, None).pack().unwrap();
...
fn python_fixture_parses_and_repacks_without_stamp_mutation() {
    ...
    let (ts, entries): (f64, Vec<serde_bytes::ByteBuf>) = rmp_serde::from_slice(&fixture).unwrap();
...
fn direct_message_is_not_queued_for_propagation() {
    let envelope = deterministic_wire(None, None).pack().unwrap();
...
fn announce_slots_parse_known_capabilities() {
    let parsed = parse_announce_slots(&[0x01, 0x03, b'a', b'b', b'c', 0x05, 0x01, 0x09]).unwrap();
    assert_eq!(parsed[0], AnnounceSlot { id: 0x01, value: b"abc".to_vec() });
}

#[test]
fn announce_parser_handles_unknown_tlv_without_panic() {
    assert_eq!(parse_announce_slots(&[0x99, 0x02, 0xAA, 0xBB]).unwrap()[0].id, 0x99);
}
...
fn regenerate_fixtures_when_env_set() {
    ...
    let no_stamp = deterministic_wire(None, None).pack().unwrap();
    let with_stamp = deterministic_wire(Some(vec![0x44; 32]), None).pack().unwrap();
    let fields = Some(Value::Map(vec![(Value::Integer(99.into()), Value::String("meta".into()))]));
    let with_meta = deterministic_wire(Some(vec![0x44; 32]), fields).pack().unwrap();
