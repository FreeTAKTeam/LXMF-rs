...
impl AutoRuntimePlan {
    /// Plans against the interfaces the host has right now.
    pub fn from_system(
        config: AutoInterfaceConfig,
        device_filter: AutoInterfaceDeviceFilter,
    ) -> Result<Self, String> {
        let candidates = enumerate_link_local_candidates()?;
        Ok(Self::from_candidates(config, device_filter, candidates))
    }

    /// Plans against a candidate set the caller already holds: a test's
    /// fixture, or an embedder that enumerates interfaces itself.
    pub fn from_candidates(
        ...
        let adopted_devices = device_filter.adopt_devices(&candidates, platform);
...
/// The host's link-local IPv6 candidates: every interface that is up, not
/// loopback, and carries a link-local address.
pub fn enumerate_link_local_candidates() -> Result<Vec<AutoInterfaceDeviceCandidate>, String> {
...
pub fn auto_carrier_runtime_json(
...
impl AutoRuntimeStatusHandle {
    pub fn from_startup_plan(plan: &AutoStartupPlan) -> Self {
    ...
    pub fn record_adopted_interface_change(&self, change: &AutoAdoptedInterfaceChange) {
        let mut guard = self.inner.lock().expect("auto runtime status mutex poisoned");
        match change {
