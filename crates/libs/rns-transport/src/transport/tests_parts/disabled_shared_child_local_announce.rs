#[tokio::test]
async fn disabled_shared_daemon_does_not_transit_announce_for_local_destination() {
    let identity = PrivateIdentity::new_from_rand(OsRng);
    let mut config = TransportConfig::new("disabled-local-announce", &identity, false);
    ...
    let destination = transport
        .add_destination(
            identity,
            DestinationName::new("lxmf", "disabled-local-announce"),
        )
        .await;
    ...
    assert!(
        ...
        "a transport-disabled shared daemon must not transit the announce to a sibling client"
    );
