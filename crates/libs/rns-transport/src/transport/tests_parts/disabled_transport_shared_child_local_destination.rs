#[tokio::test]
async fn disabled_shared_daemon_delivers_local_link_request_only_to_requesting_child() {
    let identity = PrivateIdentity::new_from_rand(OsRng);
    let mut config = TransportConfig::new("disabled-shared-local-destination", &identity, false);
    ...
    let destination = transport
        .add_destination(
            ...
            DestinationName::new("lxmf", "disabled-shared-local"),
        )
        .await;
    ...
    assert!(
        ...
        "local delivery must not broadcast or transit-forward to another child"
    );
