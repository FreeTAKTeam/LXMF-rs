...
use super::worker_boundary::{
    submit_outbound_encrypt_batch, OutboundEncryptBatchItem, PacketWireBatchItem, WorkerBackend,
    WorkerError,
};
...
mod tests {
    ...
    impl WorkerBackend for CapturingBatchBackend {
        fn submit(&self, job: WorkerJob) -> WorkerJobFuture<'_> {
            ...
            Box::pin(async move {
                let WorkerJobKind::OutboundEncryptBatch { items } = job.kind else {
                    return Err(WorkerError::InvalidJob {
                        message: "expected outbound encrypt batch".to_string(),
                    });
                };
                ...
                let items = items
                    ...
                    .collect();
                Ok(WorkerResult { id: job.id, kind: WorkerResultKind::PacketWireBatch { items } })
            })
        }
    }
