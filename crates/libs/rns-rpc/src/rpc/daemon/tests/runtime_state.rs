...
impl OutboundBridge for ConcurrentDeliveryBridge {
    fn deliver(
        &self,
...
#[test]
fn queued_inbound_raw_accept_uses_storage_writer_lane() {
    let daemon = RpcDaemon::test_instance();
    daemon
        .accept_inbound_with_raw_queued(
            MessageRecord {
                id: "queued-inbound".to_string(),
    ...
    let flush = daemon
        ...
        .expect("flush receive");
    assert!(flush.error.is_none());

    let messages = daemon
        .handle_rpc(RpcRequest { id: 6, method: "list_messages".to_string(), params: None })
        .expect("list messages")
        .result
        .expect("list result");
    let items = messages["messages"].as_array().expect("message items");
    assert!(items.iter().any(|item| item["id"] == json!("queued-inbound")));
    assert!(items.iter().any(|item| item["id"] == json!("flush-after-queued-inbound")));
}
...
#[test]
fn delivery_worker_pool_starts_second_delivery_while_first_is_blocked() {
    let store = MessagesStore::in_memory().expect("in-memory store");
    let (started_tx, started_rx) = std_mpsc::channel();
    let (release_tx, release_rx) = std_mpsc::channel();
    let daemon = RpcDaemon::with_store_and_bridges(
        store,
        "delivery-pool-node".to_string(),
        Some(Arc::new(ConcurrentDeliveryBridge::new(started_tx, release_rx))),
        None,
    );

    for (id, request_id) in [("pool-delivery-1", 14), ("pool-delivery-2", 15)] {
        let response = daemon
            .handle_rpc(rpc_request(
                request_id,
                "send_message_v2",
    ...
    let second = started_rx
        ...
        .expect("second delivery should start while first remains blocked");

    assert_eq!(first, "pool-delivery-1");
    assert_eq!(second, "pool-delivery-2");
