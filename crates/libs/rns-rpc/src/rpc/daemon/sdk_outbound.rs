...
const OUTBOUND_DELIVERY_WORKERS: usize = 4;
...
impl RpcDaemon {
    pub(super) fn spawn_outbound_delivery_worker(
        ...
        for worker_index in 0..OUTBOUND_DELIVERY_WORKERS {
            let bridge = bridge.clone();
            let store = store.clone();
            let delivery_traces = delivery_traces.clone();
            let delivery_status_lock = delivery_status_lock.clone();
            let rx = rx.clone();
            std::thread::Builder::new()
                .name(format!("rpc-outbound-delivery-worker-{worker_index}"))
                .spawn(move || loop {
                    let command = {
                        let rx = rx.lock().expect("outbound delivery receiver mutex poisoned");
                        rx.recv()
                    };
