...
use rns_rpc::{
    ControlEnvelope, EventSinkBridge, MessageRecord, MessagesStore, OutboundBridge,
    OutboundDeliveryOptions, RpcDaemon, RpcEvent, RpcEventSinkEnvelope, RpcRequest,
};
...
fn bench_message_record(id: String, timestamp: i64) -> MessageRecord {
    MessageRecord {
        id,
...
fn bench_message_store_insert(c: &mut Criterion) {
    let store = MessagesStore::in_memory().expect("in-memory store");
    let sequence = AtomicU64::new(0);
    c.bench_function("rns_rpc/message_store_insert", |b| {
        b.iter(|| {
            let seq = sequence.fetch_add(1, Ordering::Relaxed);
            let record = bench_message_record(format!("bench-store-{seq}"), seq as i64);
            store.insert_message(black_box(&record)).expect("message insert should succeed");
        });
    });
    black_box(store);
}
...
fn bench_control_boundary_envelope(c: &mut Criterion) {
    let request = rpc_request(77, "daemon_status_ex", json!({}));
    let response = RpcDaemon::test_instance().handle_rpc(request.clone()).expect("status response");
    let request_envelope = ControlEnvelope::request(1, request);
    let response_envelope = ControlEnvelope::response(2, response);
    let request_frame = request_envelope.encode_frame().expect("request frame");
    let response_frame = response_envelope.encode_frame().expect("response frame");

    c.bench_function("rns_rpc/control_boundary_envelope", |b| {
        b.iter(|| {
            let request = ControlEnvelope::decode_frame(black_box(request_frame.as_slice()))
                .expect("decode request frame");
            let encoded_request = request.encode_frame().expect("encode request frame");
            let response = ControlEnvelope::decode_frame(black_box(response_frame.as_slice()))
                .expect("decode response frame");
            let encoded_response = response.encode_frame().expect("encode response frame");
...
criterion_group!(
    ...
    bench_control_boundary_envelope
);
