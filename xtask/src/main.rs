...
const PERF_BUDGETS: &[PerfBudget] = &[
    ...
    PerfBudget {
        benchmark: "rns_core_identity_encrypt_batch_64",
    ...
    PerfBudget {
        benchmark: "rns_core_identity_encrypt_batch_64_parallel",
        max_p50_ns: 2_500_000.0,
        max_p95_ns: 3_500_000.0,
        max_p99_ns: 4_500_000.0,
        min_throughput_ops_per_sec: 350.0,
    },
    PerfBudget {
        benchmark: "rns_core_identity_decrypt_batch_64",
        max_p50_ns: 5_000_000.0,
        max_p95_ns: 7_000_000.0,
        max_p99_ns: 9_000_000.0,
        min_throughput_ops_per_sec: 180.0,
    },
    PerfBudget {
        benchmark: "rns_core_identity_decrypt_batch_64_parallel",
        max_p50_ns: 1_800_000.0,
        max_p95_ns: 2_800_000.0,
        max_p99_ns: 3_800_000.0,
        min_throughput_ops_per_sec: 550.0,
    },
    PerfBudget {
    ...
    PerfBudget {
        benchmark: "reticulumd_worker_local_outbound_encrypt_batch_64",
        max_p50_ns: 8_000_000.0,
        max_p95_ns: 10_000_000.0,
        max_p99_ns: 12_000_000.0,
        min_throughput_ops_per_sec: 100.0,
...
fn run_leader_readiness_check() -> Result<()> {
    ...
    let scorecard_max_soak_failures =
        scorecard.get("max_soak_failures").and_then(|value| value.as_u64()).unwrap_or(0);
    let scorecard_max_mesh_failures =
        scorecard.get("max_mesh_failures").and_then(|value| value.as_u64()).unwrap_or(0);
    if scorecard_max_soak_failures != 0 || scorecard_max_mesh_failures != 0 {
        bail!(
            "leader readiness requires zero-failure scorecard thresholds, found max_soak={scorecard_max_soak_failures} max_mesh={scorecard_max_mesh_failures}"
        );
    }
    ...
    for client in ["MeshChatX", "Sideband", "RCH", "Columba"] {
...
- scorecard_max_soak_failures: `{scorecard_max_soak_failures}`\n\
- scorecard_max_mesh_failures: `{scorecard_max_mesh_failures}`\n\
- soak_status: `{soak_status}`\n\
- soak_failures: `{soak_failures}`\n\
- soak_mesh_failures: `{soak_mesh_failures}`\n\
- compatibility_clients_checked: `MeshChatX`, `Sideband`, `RCH`, `Columba`\n\
...
fn run_interop_matrix_check() -> Result<()> {
    ...
    let required_clients = ["lxmf-sdk", "reticulumd", "meshchatx", "sideband", "rch", "columba"];
    ...
    let external_acceptance =
        fs::read_to_string("docs/contracts/external-client-interop-acceptance-v1.md")
            .context("missing docs/contracts/external-client-interop-acceptance-v1.md")?;
    for marker in [
        "MeshChatX",
        "Sideband",
        "Columba",
        "tools/scripts/external-client-interop-gate.sh <meshchatx|sideband|columba> [client-root]",
        "Release notes may only claim external-client interoperability",
    ] {
        if !external_acceptance.contains(marker) {
            bail!("external-client acceptance contract missing marker '{marker}'");
        }
    }

    let release_template = fs::read_to_string(".github/release-template.md")
        .context("missing .github/release-template.md")?;
    for marker in [
        "## Interop Claims",
        "tools/scripts/external-client-interop-gate.sh meshchatx",
        "tools/scripts/external-client-interop-gate.sh sideband",
        "tools/scripts/external-client-interop-gate.sh columba",
        "gate-summary.json",
    ] {
        if !release_template.contains(marker) {
            bail!("release template missing external-client interop marker '{marker}'");
        }
    }
