...
pub(super) fn reset_profile(profile: &Profile, seed: u64, output: &Path) -> CaseResult {
        ...
        (profile.power_hub_env.as_deref(), profile.power_port_env.as_deref())
    {
        match (std::env::var(hub_env), std::env::var(port_env)) {
            (Ok(hub), Ok(port)) => std::process::Command::new("uhubctl")
                .args(["-l", &hub, "-p", &port, "cycle"])
                .output()
                .map(|output| {
                    if output.status.success() {
                        Ok(())
                    } else {
                        Err(format!("uhubctl exited with {}", output.status))
                    }
                })
...
pub(super) fn reset_with_retry(profile: &Profile, seed: u64, output: &Path) -> CaseResult {
    let first = reset_profile(profile, seed, output);
    if first.result != ResultClass::FailLab {
        return first;
    }
    let mut retry = reset_profile(profile, seed, output);
    retry.attempts = 2;
...
fn shell_command(command: &str) -> std::result::Result<(), String> {
    ...
    if output.status.success() {
        ...
        Err(format!("reset hook exited with {}", output.status))
    }
}
