...
| Production async/runtime paths do not hold synchronous mutex guards across awaits | PASS | `cargo run -p xtask -- security-review-check` `run_no_sync_mutex_guard_across_await_check` |
| Production async/runtime paths do not call send/dispatch through async mutex guards | PASS | `cargo run -p xtask -- security-review-check` `run_no_async_mutex_send_across_await_check`, `InterfaceManager::send_from_shared` |
