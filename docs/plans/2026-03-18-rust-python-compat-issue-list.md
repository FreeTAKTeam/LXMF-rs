...
### 20. Resource advertisement queuing semantics are missing

Area: resource pacing

Rust behavior:

- [`crates/libs/rns-transport/src/resource/manager.rs`](crates/libs/rns-transport/src/resource/manager.rs:31) through [`crates/libs/rns-transport/src/resource/manager.rs`](crates/libs/rns-transport/src/resource/manager.rs:42) advertise immediately

Python reference:

- [`Reticulum/RNS/Resource.py`](Reticulum/RNS/Resource.py:517) through [`Reticulum/RNS/Resource.py`](Reticulum/RNS/Resource.py:522) wait until the link is ready for a new resource

Impact:

- pacing and concurrency differ from Python under load

### 21. Propagation ingest/fetch is only blob storage, not router behavior

Area: propagation node behavior

Rust behavior:

- [`crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_propagation.rs`](crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_propagation.rs:109) stores `payload_hex`
- [`crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_propagation.rs`](crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_propagation.rs:154) fetches it back by transient id

Python reference:

- [`LXMF/LXMF/LXMRouter.py`](LXMF/LXMF/LXMRouter.py:2322) locally delivers matching propagated traffic
- [`LXMF/LXMF/LXMRouter.py`](LXMF/LXMF/LXMRouter.py:2335) and [`LXMF/LXMF/LXMRouter.py`](LXMF/LXMF/LXMRouter.py:2343) persist and enqueue it for peer distribution

Impact:

- Rust propagation node semantics are far below Python router behavior

### 22. Propagation announce metadata is not persisted

Area: peer model, propagation

Rust behavior:

- [`crates/apps/reticulumd/src/bin/reticulumd/announce_worker.rs`](crates/apps/reticulumd/src/bin/reticulumd/announce_worker.rs:23) extracts only limited metadata
- [`crates/libs/rns-rpc/src/rpc/daemon/init.rs`](crates/libs/rns-rpc/src/rpc/daemon/init.rs:371) discards `stamp_cost`
- [`crates/libs/rns-rpc/src/rpc/types.rs`](crates/libs/rns-rpc/src/rpc/types.rs:497) lacks fields for transfer limit, sync limit, stamp flexibility, peering cost, and related metadata

Python reference:

- [`LXMF/LXMF/Handlers.py`](LXMF/LXMF/Handlers.py:45) parses and applies those fields
- [`LXMF/LXMF/LXMPeer.py`](LXMF/LXMF/LXMPeer.py:71) and [`LXMF/LXMF/LXMPeer.py`](LXMF/LXMF/LXMPeer.py:192) persist them

Impact:

- Rust peer and propagation decisions lack information Python depends on

### 23. Autopeering logic is too shallow

Area: peer lifecycle

Rust behavior:

- [`crates/libs/rns-rpc/src/rpc/daemon/init.rs`](crates/libs/rns-rpc/src/rpc/daemon/init.rs:375) and [`crates/libs/rns-rpc/src/rpc/daemon/init.rs`](crates/libs/rns-rpc/src/rpc/daemon/init.rs:550) use a basic hop-threshold policy

Python reference:

- [`LXMF/LXMF/Handlers.py`](LXMF/LXMF/Handlers.py:69) and [`LXMF/LXMF/Handlers.py`](LXMF/LXMF/Handlers.py:83) autopeer and unpeer based on propagation-enabled state and range

Impact:

- Rust peer state can drift away from what Python would maintain

### 24. `peer_sync` and `peer_unpeer` are bookkeeping, not real LXMF peer behavior

Area: router control plane

Rust behavior:

- [`crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_messages.rs`](crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_messages.rs:271) mostly upserts a row and counters
- [`crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_messages.rs`](crates/libs/rns-rpc/src/rpc/daemon/dispatch_legacy_messages.rs:327) mostly marks a row unpeered

Python reference:

- [`LXMF/LXMF/LXMPeer.py`](LXMF/LXMF/LXMPeer.py:267), [`LXMF/LXMF/LXMPeer.py`](LXMF/LXMF/LXMPeer.py:319), and [`LXMF/LXMF/LXMPeer.py`](LXMF/LXMF/LXMPeer.py:381) implement path discovery, backoff, link setup, peering checks, offer exchange, and transfer

Impact:

- Rust control-plane RPC names do not correspond to Python behavior

### 25. Remote peer sync rejects first-contact peers

Area: router control plane

Rust behavior:

- [`crates/apps/reticulumd/src/bin/reticulumd/inbound_worker.rs`](crates/apps/reticulumd/src/bin/reticulumd/inbound_worker.rs:253) rejects unknown peers before sync

Python reference:

- [`LXMF/LXMF/Handlers.py`](LXMF/LXMF/Handlers.py:56) and [`LXMF/LXMF/Handlers.py`](LXMF/LXMF/Handlers.py:72) create or refresh peers from announces before sync

Impact:

- Rust blocks legitimate first-contact peer sync flows

### 26. Path-response handling drops the original path-request tag

Area: path discovery

Rust behavior:

- [`crates/libs/rns-transport/src/transport/path.rs`](crates/libs/rns-transport/src/transport/path.rs:26) answers local path requests with `path_response(OsRng, None)`
- the request tag is decoded in [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:73) but not passed into the response path

Python reference:

- [`Reticulum/RNS/Transport.py`](Reticulum/RNS/Transport.py:2716) answers local destinations with `announce(path_response=True, tag=tag, attached_interface=attached_interface)`
- [`Reticulum/RNS/Destination.py`](Reticulum/RNS/Destination.py:264) caches path responses per tag and reuses the tagged announce data

Impact:

- Rust path responses are not correlated to the original Python request tag
- multipath and duplicate path-request semantics diverge from the reference

### 27. Recursive path forwarding regenerates tags instead of preserving them

Area: path discovery

Rust behavior:

- [`crates/libs/rns-transport/src/transport/path.rs`](crates/libs/rns-transport/src/transport/path.rs:69) calls `generate_recursive(..., None)`
- [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:218) then generates a new random tag when `None` is supplied

Python reference:

- [`Reticulum/RNS/Transport.py`](Reticulum/RNS/Transport.py:2810) forwards recursive path requests using the previously extracted tag

Impact:

- Rust breaks Python's loop-avoidance and path-correlation behavior for recursive discovery

### 28. Path-request duplicate suppression has no bounded lifetime

Area: path discovery

Rust behavior:

- [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:79) stores seen `(destination, tag)` pairs in a cache
- [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:110) uses it for duplicate rejection
- there is no expiry path for that cache

Python reference:

- [`Reticulum/RNS/Transport.py`](Reticulum/RNS/Transport.py:596) bounds retained path-request tags with `max_pr_tags`

Impact:

- old path-request tags can be suppressed forever in Rust instead of aging out

### 29. Recursive path-request throttling is global, not interface-aware

Area: path discovery, interface behavior

Rust behavior:

- [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:149) ignores the `on_iface` argument
- [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:186) and [`crates/libs/rns-transport/src/transport/path_requests.rs`](crates/libs/rns-transport/src/transport/path_requests.rs:195) use transport-global caps and queue length

Python reference:

- [`Reticulum/RNS/Transport.py`](Reticulum/RNS/Transport.py:2562) through [`Reticulum/RNS/Transport.py`](Reticulum/RNS/Transport.py:2585) enforce announce caps and queue checks on the specific outgoing interface

Impact:

- Rust recursive discovery pacing does not match Python on multi-interface nodes

### 30. Announce ingress limiting and held-announce release behavior are missing

Area: announce handling, interface behavior

Rust behavior:

- no equivalent announce hold/release or ingress-rate control appears in [`crates/libs/rns-transport/src/transport/jobs.rs`](crates/libs/rns-transport/src/transport/jobs.rs:1) or the surrounding transport path

Python reference:

- [`Reticulum/RNS/Interfaces/Interface.py`](Reticulum/RNS/Interfaces/Interface.py:170) holds announces
- [`Reticulum/RNS/Interfaces/Interface.py`](Reticulum/RNS/Interfaces/Interface.py:176) releases held announces based on current ingress rate and hop count

Impact:

- announce flood control and preferred-release semantics diverge from Python

### 31. Outgoing announce queue pacing is not Python-compatible

Area: announce handling, interface behavior

Rust behavior:

- announces are retransmitted from a transport-level table in [`crates/libs/rns-transport/src/transport/announce_table.rs`](crates/libs/rns-transport/src/transport/announce_table.rs:218)
- there is no per-interface announce queue equivalent

Python reference:

- [`Reticulum/RNS/Interfaces/Interface.py`](Reticulum/RNS/Interfaces/Interface.py:246) maintains a paced per-interface announce queue and transmits the best queued announce first

Impact:

- Rust announce pacing and fairness differ from Python under load
