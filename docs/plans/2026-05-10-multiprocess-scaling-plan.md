...
1. Router/control process
   ...
   measured p50 52088.44 ns, p95/p99 56837.36 ns, throughput 19198.12 ops/sec,
   ...
   pool hop. The latest report measured p50 182573.24 ns, p95/p99 185726.61 ns,
   throughput 5477.25 ops/sec, and passed.
...
1. Extract interfaces behind a process-safe boundary while preserving the
   ...
   measured p50 1166.25 ns and passed the SDK perf budget. `reticulumd` now has
...
- Benchmarks compare single-process worker lanes against multi-process mode for
  ...
  measured 24246.94 ns p50 for local completion and 67041.00 ns p50 for the
  process round-trip. Outbound encryption is measured the same way:
  `reticulumd_worker_local_outbound_encrypt` reports 64649.19 ns p50, while
  `reticulumd_worker_stdio_outbound_encrypt_round_trip` reports 97885.42 ns
...
- IPC overhead is reported separately from crypto/resource/storage work.
  ...
  report measured 3563.25 ns p50 for that envelope.
  `rns_transport/interface_worker_ipc_envelope` separately measures interface
  worker frame plus msgpack overhead at 1171.26 ns p50.
