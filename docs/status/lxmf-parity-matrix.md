...
| `LXMF/LXMRouter.py` | `crates/libs/rns-rpc`, `crates/apps/reticulumd`, `crates/apps/lxmf-cli` | partial | Outbound modes, selected propagation nodes, direct/propagated resources, cancellation, fetch/download/sync RPCs, receipts, persistence, propagation-node side effects, retry/failure handling, delivery-announce wakeup for stored pending direct/opportunistic outbound work, CLI paper encode/decode access to the SDK paper surface, and Python live remote lifecycle coverage. | No confirmed propagation-router lifecycle blocker remains; reticulumd bridge identity/path misses still need a nonterminal deferred retry queue before later delivery announces can wake them like Python. |
...
- RPC daemon `lxmf.delivery` announces reschedule stored pending direct,
  ...
  nonmatching records. Reticulumd bridge identity/path misses still need a
  deferred retry queue to fully mirror Python's announce-triggered retry loop.
