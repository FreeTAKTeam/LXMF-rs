...
| RNS-1.5.0-DELTA | `RNS/Reticulum.py`, `RNS/Identity.py`, `RNS/Transport.py`, `RNS/Link.py`, `RNS/Channel.py`, `RNS/Buffer.py`, `RNS/Discovery.py`, `RNS/Interfaces/*`, `RNS/Utilities/*` | RNS runtime, transport, RPC, and tools agents | Maintain the completed surface against exact RNS 1.5.0 while preserving explicit hardware and external-client boundaries. Bounded priority ingress, early filtering/accounting, path-request batching, blackhole filtering, full negotiated MDU, discovery operator address, telemetry, and adaptive timeout slices are implemented. | Focused unit/simulation tests, daemon/RPC/CLI status smokes, pinned-Python behavior probes at `e32d4df754a7b87b1bf1bb0d08675d12ff505ae6`, and a strict 1,839-entry inventory with no partial or unmapped entries. | Physical interface matrices, public-network soak, and external-client validation remain evidence follow-ups. |
...
## RNS 1.5.0 parity work table

The exact pinned inventory currently reports **1,838 complete, 0 partial, and
1 not-applicable entry across 1,839 entries**, with zero unmapped entries. The
