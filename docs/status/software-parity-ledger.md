...
generated callable-status inventory. Hardware/HIL evidence, prepared-host
device matrices, and external-client compatibility claims are explicitly
deferred and must not be used to promote implementation rows.
...
| --- | --- | --- | --- | --- | --- |
| RNS-RUNTIME-RETICULUM | `RNS/Reticulum.py` | RNS runtime agent | Close daemon/runtime gaps that do not depend on physical interfaces: config reload semantics, runtime mutation behavior, persistence boundaries, shared-instance behavior, graceful shutdown, RPC state visibility, and failure reporting. | Focused `reticulumd` integration tests, including `set_interfaces`/`reload_config` hot-apply policy for TCP clients; host-bound or device-bound TCP servers with loopback, wildcard, concrete, hostname, and IPv4/IPv6-preference coverage; and explicit or device-bound UDP listener, peer, multicast-bind, and multicast-forward records with Python-style device broadcast defaults, while treating startup-only interface families plus partial-target and out-of-range-target UDP shapes as restart-required or invalid and rejecting duplicate TCP server or UDP binds before mutation; daemon/RPC status snapshot tests including unified legacy `status`/`daemon_status_ex` runtime snapshot fields, Reticulum path-table restore `ok`/`error` metadata, restart-safe blackholed-identity additions and removals with associated cached-path eviction through the live daemon/transport bridge, hot-applied TCP server runtime iface plus live `_runtime.tcp.listener_status`, and hot-applied UDP runtime iface plus live `_runtime.udp.status` counter refresh; local shared-instance TCP/Unix smokes; pinned Python runtime interop when it exercises daemon behavior directly. | Device startup matrices, platform-specific prepared-host runs, external-client runtime validation. |
...
| RNS-1.5.2-DELTA | `RNS/Reticulum.py`, `RNS/Identity.py`, `RNS/Transport.py`, `RNS/Link.py`, `RNS/Channel.py`, `RNS/Buffer.py`, `RNS/Discovery.py`, `RNS/Interfaces/*`, `RNS/Utilities/*` | RNS runtime, transport, RPC, and tools agents | Maintain the completed surface against exact RNS 1.5.2 while preserving explicit hardware and external-client boundaries. RNS 1.5.1/1.5.2 queue defaults, empty keepalive handling, shared-instance dataplane exclusion, IFAC framing, profiling status, and the stream-resource owned-buffer equivalent are implemented. | Focused unit/simulation tests, daemon/RPC/CLI status smokes, pinned-Python behavior probes at `ea98db4f53dcf0defc0e71a16e60d28b1229c4e6`, and a strict 1,858-entry inventory with no partial or unmapped entries. | Physical interface matrices, public-network soak, raw IFAC carrier configuration, and external-client validation remain evidence follow-ups. |
| RNS-1.5.0-DELTA (historical) | `RNS/Reticulum.py`, `RNS/Identity.py`, `RNS/Transport.py`, `RNS/Link.py`, `RNS/Channel.py`, `RNS/Buffer.py`, `RNS/Discovery.py`, `RNS/Interfaces/*`, `RNS/Utilities/*` | RNS runtime, transport, RPC, and tools agents | Preserve the v0.10.0 RNS 1.5.0 baseline and its exact release-boundary evidence. | Historical pinned-Python evidence at `e32d4df754a7b87b1bf1bb0d08675d12ff505ae6` and the 1,839-entry inventory. | Superseded for active development by RNS 1.5.2. |
| RNS-1.4.2-DELTA (historical) | `RNS/Reticulum.py`, `RNS/Destination.py`, `RNS/Transport.py`, `RNS/Link.py`, `RNS/Interfaces/*`, `RNS/Utilities/*` | RNS runtime, transport, and tools agents | Preserve the published v0.9.9 compatibility record. | Pinned Python evidence at `b48b96e61676504e0a4e527b33b9a0b4495c6872` and the historical 1,811-entry inventory. | Superseded for active development by RNS 1.5.0. |

## RNS 1.5.2 parity work table

The exact pinned inventory currently reports **1,857 complete, 0 partial, and
1 not-applicable entry across 1,858 entries**, with zero unmapped entries. The
table records the demonstrated implementation slices and keeps hardware and
external-client evidence explicit.

| Python symbol / contract | Rust surface | Status | Risk/dependencies | Focused evidence |
| --- | --- | --- | --- | --- |
| `RNS.Destination.Destination.set_max_request_size` | `rns-core`/`rns-transport` destination request limit | complete | Wire rejection must happen before request dispatch. | Destination default/clear regression; transport direct-request and resource-advertisement limit tests. |
| `RNS.release_1_4_2.request_response_size_limits` | Link request registration and direct/resource response enforcement | complete | One-shot request correlation and malformed-envelope compatibility. | `identify_packet_tests` bounded direct-response tests; resource response cancellation tests. |
| `RNS.Resource.py` serving window | Resource sender `receiver_min_consecutive_height` collision-guard window | complete for serving-window contract | Collision-list regeneration and cross-implementation transfer evidence remain narrower follow-up concerns. | `resource_sender_serves_only_inside_the_reticulum_collision_guard_window`. |
| `RNS.release_1_4_2.interface_gravity` | `LocalInterface` gravity, policy-aware announce/path replacement | complete | Interface-specific policy must survive virtual/shared copies. | `equal_announce_prefers_higher_gravity_interface`; interface-manager gravity tests. |
| `RNS.release_1_4_2.dynamic_path_rebalancing` | Path-table freshness, random-blob, hop, latency, and gravity replacement | complete | Live multi-interface roaming remains software-smoke scope. | Path-table replacement and announce policy tests. |
| `RNS.release_1_4_2.boundary_path_requests` | Mode-gated recursive path-request forwarding | complete | Requires interface-mode metadata at forwarding boundaries. | Boundary unknown-discovery forwarding and mode-gating tests. |
| `RNS.Interfaces.BackboneInterface.blocked_ip_count/list` | TCP fast-flap tracker and listener status JSON | complete | Backbone startup defaults are configured; physical/public network evidence is deferred. | Fast-flap tracker unit test, TCP listener status tests, `reticulumd` no-default-features check. |
| `RNS.Reticulum` 1.4.2 runtime accessors | Typed runtime policy, autoconnect plan, `is_blackholed`, local hops, static identity, blackhole interval | complete for exposed typed policy surfaces | Daemon application of every autoconnect policy remains an integration follow-up. | Runtime-policy config tests; autoconnect policy test; blackhole RPC validation. |
| `RNS.Transport` lifecycle helpers | Interface hashes, shared-instance marker, hop-delta decision, queue clearing, add/remove wrappers | complete | Cross-process shared-instance persistence remains outside this focused slice. | `transport_runtime_helpers_match_shared_instance_and_hop_delta_policy`; transport lifecycle tests. |
| `RNS.Reticulum.Reticulum.rpc_return` | `reticulum-rs-rpc::RpcDaemon::rpc_return` | complete | The Rust helper returns the exact daemon connection frame; the caller owns the write. | `rpc_return_matches_the_legacy_messagepack_frame_boundary`. |
| `RNS.Utilities.rngit.client.*` | `rns-tools::rngit::ReticulumGitClient` | complete | Live Reticulum attachment is a transport adapter boundary; request framing and all client operation families are implemented. | Pinned URL/permission probes plus local service round trips for repository, release, permission, and work operations. |
| `RNS.Utilities.rngit.server.*` | `rns-tools::rngit::ReticulumGitNode` | complete | Physical/public-network validation remains outside the software-only parity claim. | Pinned handler/permission probes plus local Git repository, release, permission, and work-service tests. |
...
| RNS-RESOLVER | `RNS/Resolver.py` | RNS resolver agent | Complete. The pinned Python `resolve_identity` surface is intentionally a no-op, while Rust preserves absence and additionally implements cache and persisted-announce identity resolution. | Focused Rust resolver tests, active/tunnel restore regressions, daemon bootstrap/status coverage, and `python_resolver_reference_probe.py` in the pinned Python workflow. | None for parity against the pinned `Resolver.py` surface. |
