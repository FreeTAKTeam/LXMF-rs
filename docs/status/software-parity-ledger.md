...
| RNS-1.4.2-DELTA | `RNS/Reticulum.py`, `RNS/Destination.py`, `RNS/Transport.py`, `RNS/Link.py`, `RNS/Interfaces/*`, `RNS/Utilities/*` | RNS runtime, transport, and tools agents | Close the RNS 1.4.2 callable delta while preserving explicit hardware and external-client boundaries. Gravity, dynamic path-rebalancing, request/response-limit, boundary-request, resource-serving-window, blocked-IP, `rnstatus`, typed runtime/lifecycle, legacy RPC-return, and full `rngit` repository-service slices are implemented. | Focused unit tests, local transport/service round trips, and pinned-Python behavior probes at `b48b96e61676504e0a4e527b33b9a0b4495c6872`, with generated inventory evidence kept at 1,811 total entries and no unmapped entries. | Physical interface matrices, public-network soak, and external-client validation. |

## RNS 1.4.2 parity work table

The exact pinned inventory currently reports **1,810 complete, 0 partial, and
1 not-applicable entry across 1,811 entries**, with zero unmapped entries. The
