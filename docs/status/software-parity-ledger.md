...
| RNS-1.4.2-DELTA | `RNS/Reticulum.py`, `RNS/Destination.py`, `RNS/Transport.py`, `RNS/Link.py`, `RNS/Interfaces/*`, `RNS/Utilities/*` | RNS runtime, transport, and tools agents | Close the RNS 1.4.2 delta: gravity-based route preference, authenticated dynamic path rebalancing, destination request and response limits, boundary path-request rules, autoconnect mode/gravity/announces-to-internal policy, blocked-IP interface statistics, and `rnstatus` gravity display/sorting. | Focused unit tests for each policy edge, pinned-Python behavior probes at `b48b96e61676504e0a4e527b33b9a0b4495c6872`, daemon status assertions for `reticulum.parity`, and loopback transport/interface integration tests. | Physical interface matrices, public-network soak, and external-client validation. |
