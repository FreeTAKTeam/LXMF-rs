...
| LXMF-HANDLERS | `LXMF/Handlers.py` | LXMF handlers agent | Improve delivery, announce, propagation app-data, receipt, negative/drop, and router-coupled side-effect observability. | Daemon bridge tests; handler callback tests; receipt and drop-state regressions including direct packet/resource and propagated local-delivery drops; structured event/status assertions; pinned Python handler behavior fixtures. | External-client callback behavior, live mobile notification flows, hardware-dependent delivery paths. |
