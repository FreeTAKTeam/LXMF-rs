...
| LXMF-ROUTER | `LXMF/LXMRouter.py` | LXMF router agent | Finish non-propagation router convenience behavior that affects supported SDK/daemon flows while preserving the completed propagation-router lifecycle baseline. | Focused daemon/RPC tests; typed ZeroMQ SDK tests including paper decode metadata; local direct/opportunistic/paper/propagated mode tests; pinned Python LXMD remote lifecycle interop when protocol behavior is touched. | Sideband, MeshChatX, Columba compatibility claims; manual client workflows; broad live store-and-forward soak. |
