...
| LXMF-ROUTER | `LXMF/LXMRouter.py` | LXMF router agent | Finish non-propagation router convenience behavior that affects supported SDK/daemon flows while preserving the completed propagation-router lifecycle baseline. | Focused daemon/RPC tests, including queued/pre-handoff `app.delivery.cancel` acceptance, persisted cancelled status, delivery trace/event/lifecycle observability, no later bridge handoff, and `announce_received_wakes_pending_direct_and_opportunistic_outbound` coverage for Python-style `lxmf.delivery` announce wakeup of stored pending direct/opportunistic outbound work; typed ZeroMQ SDK tests including paper decode metadata, direct cancel result variants, and envelope cancel payload/extension preservation; `lxmf`/`lxmf-cli` paper encode/decode CLI tests that reach the typed SDK paper surface; local direct/opportunistic/paper/propagated mode tests; pinned Python LXMD remote lifecycle interop when protocol behavior is touched. | Sideband, MeshChatX, Columba compatibility claims; manual client workflows; broad live store-and-forward soak. |
