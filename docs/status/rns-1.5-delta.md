...
| 19 | Adaptive `rncp`, `rnpath`, `rnprobe`, and `rnx` timeout | bounded where network discovery exists: `rnpath` queries medium timeout; native `rnprobe` carries explicit path/probe timeout and computes a bounded total RPC read window; `rnx` Reticulum path scenarios invoke `rnpath`; the repository's root-scoped deterministic `rncp` has no network/link timeout | `cargo test -p rns-tools adaptive_timeout`; `cargo test -p rns-tools --test rnprobe_cli`; `cargo test -p rns-tools --bin rnprobe`; `cargo test -p rns-tools --bin rncp` | The native `rnprobe` packet workflow is software-tested but still lacks pinned-Python utility exchange and public/physical-link timing evidence; Rust `rncp` is deliberately a local deterministic workflow, not a live RNS file-transfer client. |
