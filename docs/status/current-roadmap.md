...
- Static peer activation now resolves existing peers case-insensitively before
  ...
  under the stored peer id instead of the configured spelling. Static
  path-response announce suppression now applies the same stored-peer identity
  rule, so a different-case path response cannot refresh an already active
  static peer's limits, liveness, or metadata.
...
- Propagation peer maintenance waiting-pool selection now matches Python's
  capped unknown-speed additions: zero-transfer-rate peers are considered, but
  only up to the fastest-pool size, preserving Python's candidate weighting.
...
- Peer record restore and export now accept and emit Python's serialized
  ...
  LXMPeer queue marks used for later sync decisions. Peer record export now
  also serializes Python-named propagation transfer limits in Python's float
  kilobyte units and sync limits in Python's integer kilobyte units while
  retaining byte-valued `transfer_limit` and `sync_limit` aliases for Rust
  status/API consumers.
