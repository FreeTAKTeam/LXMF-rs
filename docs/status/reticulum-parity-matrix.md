...
| `RNS/Resolver.py` | `crates/libs/rns-transport` | partial | Resolver helpers, cached lookup behavior, restored path-table identity lookup from cached announces, cacheless path save filtering, and Python-format stale path-table row suppression exist. | Full resolver/discovery surface parity is not established. |
