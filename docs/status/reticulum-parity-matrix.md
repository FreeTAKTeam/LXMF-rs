...
| `#608 malformed IFAC startup reporting` | `reticulumd` config bootstrap, `list_interfaces` | verified | unit | Missing IFAC credentials and an invalid seven-bit IFAC tag size keep the configured UDP interface out of transport startup and expose one fixed, credential-free failed-startup diagnostic. | Covers the tested malformed IFAC credential and tag-size cases only; general config-parse/startup reporting and remaining carrier-family/platform/physical acceptance remain open. |
...
| `#608 TCP IFAC invalid-size startup diagnostic` | `reticulumd` config bootstrap, `list_interfaces` | verified | unit | An enabled TCP listener configured with `ifac_size = 7` remains visible as a failed-startup diagnostic; the error reports the accepted `8..=512` range without including the configured credential marker. | One malformed TCP configuration shape; broader startup/configuration errors, listener restart, carrier-family/platform, and physical acceptance remain open. |
