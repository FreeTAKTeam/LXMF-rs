...
| `#611 rnpath path-table listing` | `crates/libs/rns-transport/src/transport/tests_parts/path_table_api.rs`; `crates/libs/rns-rpc/src/rpc/daemon/tests/path_rpc.rs`; `crates/apps/reticulumd/src/bin/reticulumd/bridge_path_lookup_extra_tests.rs`; `crates/apps/rns-tools/src/bin/shared/rnpath.rs` | verified | unit, mock-rpc | `get_path_table` returns Python-shaped route fields, applies `max_hops`, and the CLI sorts by interface then hop count; the positional destination filters human output but JSON retains all rows like pinned Python. Human hashes use angle-bracket hex and expiry uses local timestamp formatting. | When no configured display label exists, the bridge returns null plus `interface_hash`; this is intentionally not claimed as Python `str(interface)` parity. Software-only evidence; no non-empty live Python/Rust table trace. Broader #611 acceptance remains open. |
