...
| `#610 later split-segment cancellation` | `crates/apps/reticulumd/tests/python_channel_interop_parts/resource_faults.rs` | verified | pinned-python | The pinned Python sender accepts the first Resource part and cancels during the second segment; the Rust sender observes terminal `OutboundRejected`. | Focused fault regression only; the broader #610 segment/callback matrix remains partial. |
