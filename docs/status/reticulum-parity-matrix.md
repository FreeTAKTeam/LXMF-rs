...
| `#609 Resource after mixed-relay restart` | `crates/apps/lxmf-cli/tests/python_lxmd_remote_relay_parts/shared_instance_daemon.rs` | verified | pinned-python | After upstream Rust relay replacement and fresh multi-hop path/Link recovery, pinned Python endpoint A transfers a 131,101-byte production `RNS.Resource` to endpoint B; B verifies exact size, SHA-256, and metadata, while A observes `Resource.COMPLETE`. | One direction and one Resource after restart only. The broader two-relay post-restart test has intermittent B-to-A LXMF delivery (a 55.11-second failure followed by a 9.47-second pass); the reverse-delivery discrepancy remains unresolved, as do direct/opportunistic retry modes and broader #609 acceptance. |
| `#609 unknown shared-owner LinkRequest handoff` | `crates/libs/rns-transport/src/transport/path_parts/unknown_owner_local_link_request.rs`; `crates/libs/rns-transport/src/transport/tests_parts/shared_owner_unknown_link_request.rs` | verified | unit | A transport-enabled connected shared-instance client forwards an unknown LinkRequest only to its unique outgoing shared interface as Type 1, clamps advertised MTU, and stores the pending proof return route; ordinary unknown requests remain dropped and shared ingress is not reflected. | Handler/link-table regression only; stable end-to-end delivery through the Python shared owner after relay restart remains unverified. |
