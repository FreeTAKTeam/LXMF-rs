...
| `#613 rngit converted-media response bound` | `crates/apps/rns-tools/src/bin/rngit_parts/media_backend_tests.rs`; `crates/apps/rns-tools/src/bin/rngit_parts/page_media.rs` | verified | unit | Converted WebP output is read with the existing 32 MiB media-response cap; exact-limit output is accepted and an over-limit file is rejected to the bounded raw-media fallback. | Unit-level boundary evidence only; live oversized encoder behavior, ImageMagick 7 `magick`, `avconv`, and broader #613 acceptance remain open. |
| `#613 rngit automatic WebP backend winner` | `crates/apps/rns-tools/src/bin/rngit_parts/media.rs`; `crates/apps/rns-tools/src/bin/rngit_parts/media_backend_tests.rs`; `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_media_compression.rs` | verified | unit, pinned-python | Reuses the previous automatic backend while available, falls back to preference order if it disappears, and preserves explicit `RNGIT_MEDIA_BACKEND` precedence, matching pinned `media.py::_winner`; the direct pinned-helper regression passed against Reticulum `99de23c040d507e3fefca19e87b182302902725d`. | Selection-state parity only; ImageMagick 7 `magick`, `avconv`, and broader #613 acceptance remain open. |
| `#613 rngit unavailable forced-backend no-fallback` | `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_page_media.rs`; `.github/workflows/verify.yml` | verified | pinned-python | With `RNGIT_MEDIA_BACKEND=magick`, the server runs on an isolated `PATH` where `magick` is absent and an `ffmpeg` sentinel is available; a production Link returns the raw filename/bytes and proves the sentinel was not invoked, matching the pinned helper's recognized-but-unavailable override behavior. Local test passed at PR commit `e7563fca`; Verify runs the regression. | Selection/fallback branch only; successful ImageMagick 7 `magick`, `avconv`, visual parity, and broader #613 acceptance remain open. |
