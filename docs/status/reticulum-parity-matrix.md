...
| `#614 Pipe subprocess lifecycle` | `crates/libs/rns-transport/src/iface/pipe_tests.rs` | verified | unit | The Linux production Pipe worker observes one successful child exit, respawns once, and on interface cancellation terminates/reaps the restarted child and reports a stopped/closed state. | Bounded Linux software test only; Windows/macOS Pipe behavior, daemon-level lifecycle, peer packet exchange, and physical acceptance remain unverified. |
