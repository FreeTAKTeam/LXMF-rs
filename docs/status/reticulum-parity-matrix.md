...
| `#613 rngit automatic WebP backend winner` | `crates/apps/rns-tools/src/bin/rngit_parts/media.rs`; `crates/apps/rns-tools/src/bin/rngit_parts/media_backend_tests.rs`; `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_media_compression.rs` | verified | unit | Reuses the previous automatic backend while it remains available, falls back to preference order if it disappears, and leaves explicit override precedence intact, matching pinned `media.py::_winner`. A direct pinned-helper regression is added but was not run locally because the pinned checkout is absent. | Selection-state parity only; real encoding on other backend families and broader #613 acceptance remain open. |
...
| `#613 rngit nested image markup encoding` | `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_page_media.rs` | verified | pinned-python, simulated | A pinned Python production Link requests a nested image path containing a space; the Rust-rendered Micron media markup matches the frozen `pages.py` `quote_plus(file_path)` encoding. | One nested path case only; other template/rendering behavior and the broader #613 acceptance remain open. |
