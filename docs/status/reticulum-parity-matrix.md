...
| `RNS/Discovery.py` | `crates/libs/rns-transport`, `crates/apps/reticulumd` | partial | Announce/path discovery plus live AutoInterface discovery and peer runtime, including Python-style final-init gating before daemon discovery datagrams can add peers, and daemon `_runtime.auto.carrier_runtime.last_peer_job` visibility for expired peers, reverse peer announces, missing initial multicast echoes, carrier events, and post-job peer counts. | Public bootstrap/discovery breadth and broader real-interface churn evidence remain narrower than Python. |
