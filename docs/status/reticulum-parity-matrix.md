...
| `#611 rnprobe missing destination` | `crates/apps/rns-tools/tests/rnprobe_python_interop.rs` | verified | pinned-python, process | With only `rnstransport.probe` supplied, frozen Python prints argparse help and exits 0, whereas the Rust production process exits 1 with `rnprobe: destination hash is required` and empty stdout; neither starts networking. | One missing-argument behavior only; broader `rnprobe` option and #611 utility parity remain partial. |
