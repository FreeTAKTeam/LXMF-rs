...
| `#611 rnsh listener PTY and resize` | `crates/apps/rns-tools/tests/rnsh_process.rs`; `rnsh_parts/pty.rs` | verified | unit, process, simulated | A controlling-terminal child observes initial `31x101` and resized `42x120` dimensions; the PTY-reported size also verifies initial and updated pixel dimensions. The loopback test exercises ExecuteCommand terminal flags, client SIGWINCH, listener WindowSize handling, output, and exit. | All-terminal mode only; mixed per-stream combinations and the wider #611 rnsh matrix remain open. |
