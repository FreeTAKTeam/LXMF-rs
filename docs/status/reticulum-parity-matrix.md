...
| `#613 rngit runtime WebP CLI options` | `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_media_options.rs` | verified | pinned-python, simulated | A production `rngit` process selects a deterministic stub `ffmpeg` and forwards configured quality and maximum-dimension CLI options; a pinned Python Link verifies the WebP Resource response and the stub-recorded argv. | Runtime CLI-to-backend wiring only; real `convert`, `gm`, `ffmpeg`, and `magick` coverage is recorded separately; `avconv`, visual parity, and broader #613 acceptance remain open. |
...
| `#613 rngit WebP encoder failure fallback` | `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_media_options.rs`; `crates/apps/rns-tools/src/bin/rngit_parts/page_media.rs`; `crates/apps/rns-tools/src/bin/rngit_parts/media.rs` | verified | pinned-python, simulated | A deterministic `ffmpeg` stub exits nonzero with a diagnostic; a pinned Python production Link receives the exact 68-byte raw PNG and original filename, server stderr retains the encoder detail, and the failed conversion directory is removed. | Failure/fallback evidence for the stubbed path; successful real `ffmpeg`, `convert`, `gm`, and local `magick` runs are recorded separately; `avconv` and broader #613 acceptance remain open. |
