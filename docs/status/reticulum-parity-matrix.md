...
surface inventory produces **1,695 complete, 115 partial, and 1
not-applicable entry across 1,811 entries**.

The reopened software gaps are:

- dynamic link/path rebalancing and interface-gravity path preference;
- destination request-size and request `max_response_size` controls;
- boundary-to-boundary and boundary-to-gateway path requests;
- autoconnect mode, gravity, and announces-to-internal policy;
...
| --- | --- | --- | --- | --- | --- |
| `RNS/Reticulum.py` | `crates/libs/rns-transport`, `crates/apps/reticulumd` | partial | unit, simulated, pinned-python | Deployable daemon, configuration, propagation-node activation, persistence, RPC, graceful shutdown, unified legacy `status`/`daemon_status_ex` daemon runtime snapshot visibility, daemon/RPC runtime status for Reticulum path-table restore success or failure, Reticulum-style blackholed identity list/add/remove RPC state with restart-safe persistence and associated cached-path eviction, path drop and drop-all-via runtime RPC mutation, announce-queue clearing, a 16-timestamp announce-rate table, 512-entry packet RSSI/SNR/Q caches, Python-compatible persisted discovered-interface listing with source/type/endpoint validation and age transitions, multiple live interfaces, runtime `set_interfaces`/`reload_config` hot-apply for TCP/UDP/Pipe surfaces, and typed Python-default runtime policy accessors for implicit proofs, MTU discovery, remote management, probes, discovery stamps/sources/autoconnect, and blackhole publication/sources. | RNS 1.4.2 autoconnect mode/gravity/announces-to-internal policy and newly exposed lifecycle helpers remain partial. |
...
| `RNS/Destination.py` | `crates/libs/rns-core`, `crates/libs/rns-transport` | partial | unit, pinned-python | Destination hashing, descriptors, announces, proof generation and validation, ratchets, and known-key stability checks. Single-destination Data delivery proofs are correlated through the packet cache before identity verification. | RNS 1.4.2 `set_max_request_size` and request `max_response_size` controls are not yet exposed. |
...
| `RNS/Transport.py` | `crates/libs/rns-transport`, `crates/apps/reticulumd` | partial | unit, simulated, pinned-python | Path and announce handling, path replacement/state/await semantics, routed links/resources/receipts/tunnels, next-hop bitrate/MTU/latency formulas, interface prioritization/detach/lookup, destination registration lifecycle, discovery and blackhole state, shared-instance behavior, packet filtering, Python-format forced disk packet cache and announce cleanup, MessagePack packet-hash/path/tunnel persistence, runtime jobs and graceful shutdown. Existing deterministic evidence also covers known-path response precedence, roaming suppression/grace, recursive discovery, link MTU signalling, restore/restart, scoped path requests, pacing, duplicate suppression, bound-interface link fan-out with structured partial-failure reports, and `enable_transport = false` suppression of Link Request and established-link transit forwarding while local destinations remain reachable. | RNS 1.4.2 gravity selection, dynamic path rebalancing, boundary path-request rules, and newly exposed lifecycle helpers remain partial. |
| `RNS/Link.py` | `crates/libs/rns-transport` | partial | unit, pinned-python | Establishment, proof validation, bound-interface enforcement for data/channel fan-out, RTT-derived liveness, protocol close, and cleanup. | RNS 1.4.2 link-proof path rebalancing remains unimplemented. |
| `RNS/Resource.py` | `crates/libs/rns-transport` | partial | unit, simulated, pinned-python | Bounded receive allocation, advertisement validation, retries, window-local hashmap exhaustion gated on an outstanding update (matching `request_next`/`waiting_for_hmu`, with a bounded wait so a lost update cannot park the transfer), bz2 compression in both directions — inbound decompression, plus outbound auto-compression matching `Resource.__init__`'s `auto_compress` default, completing a round trip of which only the receive half was previously implemented — adaptive fragment scheduling on Python's own ladder (`WINDOW`/`WINDOW_MIN`/`WINDOW_MAX_SLOW`/`WINDOW_MAX_VERY_SLOW`/`WINDOW_MAX_FAST`/`WINDOW_FLEXIBILITY` with rate-measured ceiling changes), timeout/failure events, cancellation, cleanup, and Python-shaped split-resource sequencing with original-hash preservation, ordered reassembly, per-segment completion metadata carried by the first segment alone (matching `Resource.py`, which flags every segment as metadata-bearing so the receiver can size `total_data_size`, but prefixes the length-delimited block to segment 1 only), and final whole-resource completion. | The sender has no `receiver_min_consecutive_height` serving window (`COLLISION_GUARD_SIZE`), so a Rust↔Rust transfer cannot exercise the behaviour a Rust↔Python one does — receiver-side parity has to be evidenced against Python. |
...
| `RNS/Interfaces/*` | `crates/libs/rns-transport`, `crates/apps/reticulumd` | partial | unit, simulated, prepared-host, pinned-python, hardware-unverified | Configuration, framing, startup, reconnect, runtime status/mutation, management, and teardown are implemented for TCP/Backbone, Local, Pipe, UDP, Serial, KISS/AX.25, AutoInterface, I2P/SAM, Meshtastic, RNode serial/TCP/BLE/SPP, RNodeMulti virtual ports, Weave, Android BLE/serial, and VR-N76. Evidence includes loopback TCP/UDP/Unix/subprocess flows, PTY/fake-TCP KISS, fake-SAM and prepared-host I2P, deterministic Meshtastic faults, BLE mocks, RNode/RNodeMulti/Weave management/status state machines, reconnect/backpressure/corruption handling, and pinned-Python interface probes. | RNS 1.4.2 interface gravity and Backbone blocked-IP statistics remain partial; physical devices and public networks remain `hardware-unverified`. |
...
| `RNS/Utilities/*` | `crates/apps/rns-tools` | partial | unit, simulated, pinned-python, hardware-unverified | Canonical `rnx`, `rnsd`, `rnstatus`, `rnpath`, `rnodeconf`, `rncp`, `rnid`, `rnir`, `rnpkg`, `rnprobe`, `rnsh`, and `rngit` binaries are present. Existing `*-rs` aliases remain for status/path/RNode configuration. The utilities cover daemon delegation and status, scoped path requests, radio management, identity persistence, binary-safe root-scoped copy, probe exit status, default-deny isolated shell execution, and root-scoped Git bundle workflows; remote/link/resource behavior reuses typed daemon, SDK, and transport surfaces. | `rnstatus` does not yet expose RNS 1.4.2 gravity sorting/display or blocked-IP statistics. Rust `rngit` covers isolated local Git bundle preparation but not the Python remote client/server, repository service, or work-item API. Physical radio commands remain `hardware-unverified`. |
...
- LocalInterface TCP-loopback listener/client-attach plus Unix filesystem and
  ...
  application-level shared-instance traffic parity. LocalInterface #384 evidence
