...
| `#611 rnpath path-table listing` | `crates/apps/rns-tools/tests/rnpath_python_table.rs`; `crates/libs/rns-transport/src/transport/tests_parts/path_table_api.rs`; `crates/libs/rns-rpc/src/rpc/daemon/tests/path_rpc.rs`; `crates/apps/reticulumd/src/bin/reticulumd/bridge_path_lookup_extra_tests.rs`; `crates/apps/rns-tools/src/bin/shared/rnpath.rs` | verified | unit, mock-rpc, pinned-python, simulated | `get_path_table` returns Python-shaped route fields, applies `max_hops`, and the CLI sorts by interface then hop count; the positional destination filters human output but JSON retains all rows like pinned Python. An ignored live loopback trace compares a non-empty Rust CLI JSON row with a separate frozen-Python `get_path_table()` observer after a pinned Python peer announces: destination hash, via, and one-hop count match; expiry values are numeric. | Interface labels differ: Rust emits configured name `python-rnpath-table-peer`; frozen Python emits `TCPInterface[TCP Client Interface/127.0.0.1:<port>]`. Other interfaces and broader #611 acceptance remain open. |
