...
| `#609 exact packet/proof replay after standalone restart` | `crates/libs/rns-transport/src/transport/packet_cache.rs`; `crates/libs/rns-transport/src/transport/packet_disk_cache.rs`; `crates/libs/rns-transport/src/transport/tests_parts/packet_replay_restart.rs`; `crates/apps/reticulumd/src/bin/reticulumd/bootstrap_transport.rs` | verified | unit, pinned-python-source | The pinned Python Reticulum `99de23c040d507e3fefca19e87b182302902725d` reloads and persists concatenated 32-byte hashes in `packet_hashlist.raw`, and filters exact packet hashes including ordinary proofs for standalone transport nodes. Rust bootstrap now restores that cache before ingress, writes the same format, preserves compatibility with its earlier MessagePack file, and a fresh-transport production-ingress regression rejects the exact proof replay. | Exact cached-hash replay only; proof/retransmission classes, cache rotation/size parity, process-level transport shutdown/restart, and broader #609 acceptance remain open. |
