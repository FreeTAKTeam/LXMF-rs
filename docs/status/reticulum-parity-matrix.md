...
#610 row remains partial because broader timeout/reconnect and Python-side
file-adapter fault coverage are not yet proven; the pinned-Python suite now proves cancellation
terminal events in both directions, exact 50 MiB peak-RSS values in both
directions under a fixed release-profile process budget, and a pinned-Python
