...
| `#612 rngit permissions-get repository gates` | `crates/apps/rns-tools/src/bin/rngit_parts/{work_service.rs,issue_612_document_admin_operations_differential_tests.rs}`; pinned RNS 1.5.4 `RNS/Utilities/rngit/server.py` | verified | pinned-python | Four production-handler differentials confirm repository-admin plus repository write/interact gates precede document-level author/admin permissions for `perms/get`; they compare exact response status/body and permission-sidecar state. | Four focused handler cases only; other permission combinations, `perms/set` ordering, and broad #612 acceptance remain open. |
