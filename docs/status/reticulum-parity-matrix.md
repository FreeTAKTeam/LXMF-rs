...
| `#608 malformed IFAC startup reporting` | `reticulumd` config bootstrap, `list_interfaces` | verified | unit | Missing IFAC credentials keep the configured UDP interface out of transport startup and expose one failed startup diagnostic with a fixed, credential-free error message. | Covers malformed IFAC credentials only; other parse/startup errors and remaining carrier-family/platform/physical acceptance remain open. |
