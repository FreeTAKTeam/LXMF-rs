...
#610 row remains partial because loss/duplication/reordering/link-timeout
fault-injection coverage and peak-memory measurement are not yet proven; the
pinned-Python suite now proves cancellation terminal events in both directions,
and a pinned-Python split reader-backed transfer covers the Rust reader/file
