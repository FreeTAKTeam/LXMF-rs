...
| `#613 rngit executable template launch failure fallback` | `crates/apps/rns-tools/src/bin/rngit_parts/{pages.rs,issue_613_template_failure_tests.rs}`; `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_template_launch_failure.rs`; `docs/goals/reticulum-reference-parity-605/evidence/613-rngit-pages.md` | verified | pinned-python, production-link | A pinned Python client at the frozen revision requests `/page/index.mu` over a real TCP Reticulum Link from the Rust service with an executable `base.mu` whose interpreter is missing; the built-in footer is served and the service remains alive. This matches `pages.py::get_template` treating launch exceptions as unavailable. | Executable-template launch failure only; complete Markdown highlighting, pagination, diff/work-document rendering, and other template details remain open. |
