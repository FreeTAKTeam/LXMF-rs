...
coverage are not yet proven; daemon completion-consumer regressions verify
...
| `#610 daemon Resource timeout consumer` | `crates/apps/reticulumd/src/bin/reticulumd/inbound_worker_parts/inbound_propagation_payload_is_inges_sections/resource_terminal_consumer.rs` | verified | simulated | `production_daemon_consumer_reports_resource_timeout_and_cleans_tracking` drives a held Resource request on an active Link through timeout, verifies the correlated `resource-failed` receipt and timeout status, clears tracking, and persists the failure status. | One deterministic daemon-consumer path; broader timeout/reconnect and callback/status coverage remain partial. |
...
| `#610 daemon Resource event observability` | `crates/apps/reticulumd/src/bin/reticulumd/inbound_worker_parts` | verified | unit, simulated | Timeout failure emits one `resource-failed` receipt and peer rejection emits one `resource-rejected` terminal receipt, with resource/peer/byte metadata and tracking cleanup; duplicate terminal notifications do not emit a second receipt. Inbound progress is logged with Resource hash, Link ID, received/total bytes, and received/total parts. Test-only channel gates verify partial-progress Link teardown and public peer cancellation separately: each yields one correlated inbound failure with no completion, receipt, status transition, or delivered content. The Link-teardown payload recovers on a fresh Link; after peer cancellation, a new exact-payload Resource completes on the same active Link. | Focused daemon-consumer evidence only; broader failure and consumer callback/status matrix remains partial. |
