...
| `#613 rngit media zero-stat and tree paths` | `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_media_zero_stat.rs`; `crates/apps/rns-tools/src/bin/rngit_parts/page_media.rs`; `.github/workflows/verify.yml` | verified | pinned-python | A pinned Python Link checks empty/nonempty blob streaming and a tracked directory request. Source assertions bind the directory case to pinned `pages.py`'s `cat-file -s` then `git show` behavior; Rust returns the bounded Git tree listing with basename metadata. | Source-checked pinned-client comparison against the Rust server, not a Python-server differential; metadata/stat races and broader #613 acceptance remain open. |
