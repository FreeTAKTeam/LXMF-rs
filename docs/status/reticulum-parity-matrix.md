...
The v0.9.8 release boundary preserves this inventory. The Rust resource sender
now enforces the Python `receiver_min_consecutive_height` serving window;
collision-list regeneration and cross-implementation transfer evidence remain
narrower follow-up concerns. No hardware, public-network, or third-party-client
claim is inferred from the release candidate.
...
| `RNS/Transport.py` | `crates/libs/rns-transport`, `crates/apps/reticulumd` | partial | unit, simulated, pinned-python | Path and announce handling, gravity-aware replacement, dynamic path rebalancing, boundary path requests, path replacement/state/await semantics, routed links/resources/receipts/tunnels, next-hop bitrate/MTU/latency formulas, interface prioritization/detach/lookup, destination registration lifecycle, discovery and blackhole state, shared-instance behavior, packet filtering, Python-format forced disk packet cache and announce cleanup, MessagePack packet-hash/path/tunnel persistence, runtime jobs and graceful shutdown, and the focused lifecycle helpers. Existing deterministic evidence also covers known-path response precedence, roaming suppression/grace, recursive discovery, link MTU signalling, restore/restart, scoped path requests, pacing, duplicate suppression, bound-interface link fan-out with structured partial-failure reports, and `enable_transport = false` suppression of Link Request and established-link transit forwarding while local destinations remain reachable. | Broader callable coverage remains partial; no 1.4.2 routing residual is currently claimed in the generated inventory. |
| `RNS/Link.py` | `crates/libs/rns-transport` | partial | unit, pinned-python | Establishment, proof validation, bounded request/response correlation, bound-interface enforcement for data/channel fan-out, RTT-derived liveness, protocol close, and cleanup. | Broader link callable coverage remains partial; the focused dynamic path-rebalancing slice is implemented. |
| `RNS/Resource.py` | `crates/libs/rns-transport` | partial | unit, simulated, pinned-python | Bounded receive allocation, advertisement validation, retries, receiver-minimum collision-guard serving window, window-local hashmap exhaustion gated on an outstanding update (matching `request_next`/`waiting_for_hmu`, with a bounded wait so a lost update cannot park the transfer), bz2 compression in both directions, adaptive fragment scheduling on Python's own ladder, timeout/failure events, cancellation, cleanup, Python-shaped split-resource sequencing with original-hash preservation, ordered reassembly, per-segment completion metadata, and final whole-resource completion. | Broader resource callable coverage remains partial; collision-list regeneration and cross-implementation transfer evidence are narrower follow-ups. |
...
| `RNS/Interfaces/*` | `crates/libs/rns-transport`, `crates/apps/reticulumd` | partial | unit, simulated, prepared-host, pinned-python, hardware-unverified | Configuration, framing, startup, reconnect, runtime status/mutation, management, teardown, interface gravity, and Backbone blocked-IP statistics are implemented for the supported software surfaces. Evidence includes loopback TCP/UDP/Unix/subprocess flows, PTY/fake-TCP KISS, fake-SAM and prepared-host I2P, deterministic Meshtastic faults, BLE mocks, RNode/RNodeMulti/Weave management/status state machines, reconnect/backpressure/corruption handling, fast-flap tracker tests, and pinned-Python interface probes. | Broader interface callable coverage remains partial; physical devices and public networks remain `hardware-unverified`. |
...
| `RNS/Utilities/*` | `crates/apps/rns-tools` | partial | unit, simulated, pinned-python, hardware-unverified | Canonical `rnx`, `rnsd`, `rnstatus`, `rnpath`, `rnodeconf`, `rncp`, `rnid`, `rnir`, `rnpkg`, `rnprobe`, `rnsh`, and `rngit` binaries are present. Existing `*-rs` aliases remain for status/path/RNode configuration. The utilities cover daemon delegation and status, scoped path requests, gravity sorting/display, blocked-IP statistics, radio management, identity persistence, binary-safe root-scoped copy, probe exit status, default-deny isolated shell execution, and the full transport-neutral rngit repository, release, permission, work-item, and Git bundle workflows. Physical radio commands remain `hardware-unverified`. |
