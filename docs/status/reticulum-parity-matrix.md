...
| `#610 daemon Resource event observability` | `crates/apps/reticulumd/src/bin/reticulumd/inbound_worker_parts` | verified | unit | Timeout failure emits one `resource-failed` receipt and peer rejection emits one `resource-rejected` terminal receipt, with resource/peer/byte metadata and tracking cleanup; duplicate terminal notifications do not emit a second receipt. Inbound progress is logged with Resource hash, Link ID, received/total bytes, and received/total parts. | Focused daemon-consumer evidence only; broader failure and consumer callback/status matrix remains partial. |
