...
- AutoInterface discovery, authenticated peering, peer lifecycle, duplicate
  ...
  data-listener restart, stale outbound route pruning after restart, dynamic
...
still needs dynamic add/remove supervision for newly appearing or disappearing
adopted interfaces, which requires discovery socket listener supervision in
addition to the implemented link-local data-listener replacement reconciler.
