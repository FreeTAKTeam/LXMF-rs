...
| `#613 rngit zero-sized media pipe response` | `crates/apps/rns-tools/tests/rngit_python_interop/issue_613_media_zero_stat.rs`; `.github/workflows/verify.yml` | verified | pinned-python | A pinned Python client receives empty and nonempty tracked blobs with exact filename/content over production Links; pinned source checks confirm `pages.py` returns a `git show` pipe and `Resource.py` takes its zero-stat proxy branch. | Narrow source-checked case against the Rust server, not a Python-server differential; metadata/stat races and broader #613 acceptance remain open. |
