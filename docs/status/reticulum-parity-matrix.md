...
| `RNS/Utilities/*` | `crates/apps/rns-tools` | partial | unit, process, simulated, pinned-python, hardware-unverified | Canonical `rnx`, `rnsd`, `rnstatus`, `rnpath`, `rnodeconf`, `rncp`, `rnid`, `rnir`, `rnpkg`, `rnprobe`, `rnsh`, and `rngit` binaries cover daemon/status delegation, scoped path requests, gravity display, blocked-IP statistics, radio management, identity persistence, binary-safe copy, probe status, isolated shell execution, and transport-neutral repository/release/permission/work-item/Git-bundle workflows. A production Rust `rngit fetch` CLI now receives a bundle from the pinned Python service, verifies and imports it into a local Git ref, and checks the exact commit and binary blob. Native `rnsh` TCP/Link/Channel evidence covers authentication, command policy, stream forwarding, timeout and remote-child reaping, channel-window retry and queue failure handling; the listener now creates a controlling PTY for terminal requests, applies initial rows/columns/pixel size and later `WindowSize` updates, and binds child I/O/exit/cancellation to its Link-owned task, while all-pipe requests retain pipe-backed stdio. `rncp` has two-process Resource send/fetch and save-failure tests, authenticated pinned-Python transfer roles (including compression, overwrite and metadata-driven save), concurrent clients, restart persistence, interrupted transfers and adaptive timeout evidence. `rnpath` has live daemon RPC discovery against a pinned-Python peer. `rngit` has pinned-Python list/fetch/push/delete/create/sync/fork/mirror service coverage with bundle/ref/repository effects. `nomadnet` compatibility covers pinned-Python page/file requests, denial, rendered content, raw media Resources and live PNG-to-WebP conversion. | #611/#612/#613 remain partial pending broader failure/restart and reverse-listener roles, Python allow-list/jail/overwrite roles, remaining `rngit` page/media/error and network workflows, Reticulum-source cloning, signature validation, other rendering/conversion backends, and the complete utility behavior matrix. Rust CLI push and the full Git remote-helper flow remain open. The PTY slice is verified in all-terminal mode; mixed per-stream combinations, public/multi-hop behavior and physical evidence remain open or excluded. See `evidence/611-utilities.md`. |
| `#611 rngit production CLI fetch` | `crates/apps/rns-tools/tests/rngit_cli_python_fetch.rs`; `rngit_parts/network_fetch.rs` | verified | process, pinned-python | Separate Rust CLI and pinned Python service processes exchange a real Git bundle over Reticulum; the CLI verifies the bundle, runs local `git fetch` into the requested ref, and the regression checks the exact source commit and binary blob bytes. Reference is pinned to Reticulum `99de23c040d507e3fefca19e87b182302902725d`. | One fetch path only; Rust CLI push, full remote-helper behavior, configured initial-branch variation, and broader #611 utility acceptance remain open. |
