...
| `#612 rngit malformed work request envelope` | `crates/apps/rns-tools/src/bin/rngit_parts` | verified | unit | Missing/wrong-typed numeric repository keys and a missing operation return exact error codes/bodies through `handle_request` without changing repository/work/permission filesystem state. | Local handler-only coverage; no new mixed-peer request trace. |
