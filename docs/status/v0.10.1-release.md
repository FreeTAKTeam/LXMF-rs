...
artifacts, provenance, independent interoperability, crates.io publication,
and performance evidence are independently verified below.
...
| Tag-triggered release artifacts | PASS | Release workflow [33254264203](https://github.com/FreeTAKTeam/LXMF-rs/actions/runs/33254264203) succeeded; the public release contains 35 platform/package/SBOM, independent-evidence, and performance assets |
...
| Performance comparison | PASS WITH WARNINGS | Tag workflow [33254264175](https://github.com/FreeTAKTeam/LXMF-rs/actions/runs/33254264175) completed successfully in 1h36m23s. The release gate reports `pass_with_warnings`, with throughput `1.013x`, CPU `1.010x`, and peak RSS `1.084x` versus the same-runner v0.9.1 baseline; the only warning is 13.99% dispersion for Rust resource-sized message encode. The four checksummed performance assets are attached to the public release |
...
- The public performance JSON, HTML, raw bundle, and `SHA256SUMS` from workflow
  [33254264175](https://github.com/FreeTAKTeam/LXMF-rs/actions/runs/33254264175)
  were downloaded and passed `sha256sum -c SHA256SUMS` independently. The
  canonical copies are [`docs/performance/v0.10.1.json`](../performance/v0.10.1.json)
  and [`docs/performance/v0.10.1.html`](../performance/v0.10.1.html).
