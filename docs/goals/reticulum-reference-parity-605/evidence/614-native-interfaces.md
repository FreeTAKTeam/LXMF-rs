...
| BLE startup cancellation cleanup | The production interface worker selects both interface cancellation tokens while `runtime.startup()` is pending. On cancellation it drops the in-flight setup future, closes the runtime backend, then exits instead of remaining blocked in connect/service discovery. A deterministic backend marks a partial connection resource acquired before holding `connect()` indefinitely; cancellation verifies that cleanup releases the acquired resource and the worker exits within a one-second test bound. This is token-driven software cancellation; task abortion and native GATT cancellation are not covered. | focused fake-backend regression; native GATT behavior unverified |
...
| Interface inventory and unsupported-kind policy | A production daemon-startup regression parses an unknown kind, records exactly one startup failure, and exposes `startup_status = failed` plus the exact unsupported-kind error in interface runtime metadata. A focused strict-policy regression verifies that the same diagnostic is promoted to a startup rejection instead of being silently ignored. | deterministic daemon software regressions; interface-family/platform/live evidence remains open |
| UDP worker teardown | A Linux loopback regression starts the production UDP interface worker on an ephemeral localhost port, observes `bound` status, verifies another non-reuse socket cannot claim the live port, cancels the interface, awaits worker termination and `closed` status, then binds the released port. | focused Linux software-only cancellation/teardown test; receive/transmit interoperability and other-platform socket semantics remain separate evidence |
...
| I2P connectable accept recovery after expired SAM session | The production accept loop receives a deterministic `STREAM STATUS ... is not a STREAM session` response, recreates its SAM STREAM session, then accepts and registers an incoming peer. The test asserts one accept-reconnect transition and the live incoming-peer row. This pins session-recreation behavior already present in the runtime; it does not claim real I2P-router, public-network, IFAC, or broad family parity. | All 13 I2P-focused transport tests passed, including `i2p_accept_loop_recreates_expired_sam_session_before_accepting_peer`; strict transport Clippy, fmt, and module-size checks passed. Fake-SAM software only. |
| I2P outbound setup cancellation during a stalled SAM handshake | The production outbound peer loop selects daemon and interface cancellation while `open_sam_stream` awaits the SAM HELLO handshake. Separate deterministic fake-SAM regressions cancel the daemon token and the interface stop token while HELLO REPLY is withheld; each worker exits within one second and the server observes socket EOF. | all 16 focused I2P tests, strict transport Clippy, formatting and module-size checks passed; fake-SAM software evidence only, not router or public-network evidence |
| I2P outbound reconnect backoff after stream expiry | Pinned Reticulum `99de23c040d507e3fefca19e87b182302902725d`, `RNS/Interfaces/I2PInterface.py`, sets `I2PInterfacePeer.RECONNECT_WAIT = 15` and sleeps that interval in `reconnect()` after its established client stream ends. Rust previously reopened SAM immediately after the production HDLC stream runner returned. The production-loop fake-SAM regression now closes an established stream, times the subsequent SAM session attempt, and requires the configured backoff; the loop uses its existing cancellation-aware `reconnect_wait`. Verify runs this focused regression. | Focused local fake-SAM regression passed; PR Verify execution is pending. The test uses a shortened 120 ms configured delay, not a 15-second wall-clock wait. This is software-only; router interoperability, network soak, and the wider #614 matrix remain unverified. |
...
| AutoInterface daemon partial-activation rollback | A daemon-binary regression activates a loopback plan whose discovery listener binds successfully and whose later data listener has an invalid address. Startup fails, the daemon removes the newly registered host channel, and the already-bound discovery port is immediately reusable. This covers the daemon cleanup layer beyond the library's existing partial-bind rollback regression. | focused daemon-binary loopback regression; native carrier and cross-platform socket behavior remain unverified |
| AutoInterface competing activation and retry | The production daemon adapter starts one loopback AutoInterface, rejects a competing activation on its exclusively owned data port without removing the owner channel or leaking a new channel, then accepts the same plan after the owner is explicitly stopped. | `daemon_auto_competing_activation_preserves_owner_and_retries_after_stop` passed as a software-only daemon-binary test; native carrier and cross-platform socket behavior remain unverified |
...
Focused validation for the stalled-handshake cancellation increment:

```text
cargo test -p reticulum-rs-transport --lib i2p_ -- --nocapture
PASS (16 tests)
cargo clippy -p reticulum-rs-transport --all-targets --no-deps -- -D warnings
PASS
cargo fmt --all -- --check
PASS
...
Unsupported-interface strict-policy follow-up on the current PR #634 head:

```text
cargo test -p reticulumd --bin reticulumd strict_startup_policy_rejects_unsupported_interface_kind_explicitly -- --nocapture PASS (1 test)
cargo fmt --all -- --check                                      PASS
cargo clippy -p reticulumd --bin reticulumd --all-targets --no-deps -- -D warnings PASS
tools/scripts/check-module-size.sh                              PASS
git diff --check                                                 PASS
```
...
The UDP carrier cancellation/teardown regression ran on Linux:

```text
cargo test -p reticulum-rs-transport --lib udp_interface_cancellation_closes_worker_and_releases_bound_port -- --nocapture PASS (1 test)
```
