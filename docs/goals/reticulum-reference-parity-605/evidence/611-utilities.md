...
client behavior. No production discrepancy was exposed. This is software
loopback evidence and does not complete the broader `rnprobe` options,
failure/restart, multi-hop, public-network, or physical matrix. A new
...
| Packet probe exchange | Probe packet delivery and proof correlation between the native daemon path and the pinned Python utility, in both initiator/responder directions, plus sequential native utility reuse through one daemon | ignored `rnprobe_python_interop` (2 tests, commit `f86ecc1c`); `native_rnprobe_reuses_one_live_daemon_for_sequential_client_processes` | verified for isolated Rust daemon/Python TCP roles and sequential independent CLI clients sharing one live daemon; daemon restart, public/multi-hop, carrier-fault, and physical timing remain open |
