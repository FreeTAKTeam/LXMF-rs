...
| Complete float document ID | `work_complete_float_id_matches_pinned_python_integer_coercion_and_directory_transition` submits `doc_id=7.9` to both production handlers. Pinned Python applies `int(7.9)`, completes document `7`, and returns success; Rust previously rejected the float as an invalid document ID. Rust now reuses the shared Python-compatible document-ID coercion and matches the response plus active-to-completed directory side effect. | Focused local pinned-Python production-handler differential verified against Reticulum `99de23c040d507e3fefca19e87b182302902725d`; only `complete` with this fractional ID is covered; broad #612 work-operation matrix remains open |
...
| Malformed work comment content | Pinned Python `_work_comment` calls `.strip()` before validation; an authorized comment with array-valued `content` raises and the production handler returns `REMOTE_FAIL` / `Remote error`. Rust's production handler now maps this malformed value to the same response. `work_comment_malformed_content_matches_pinned_python_without_mutation` compares both handlers and confirms the seeded root file is unchanged and no comment file is created. | Focused local pinned-Python production-handler differential verified against Reticulum `99de23c040d507e3fefca19e87b182302902725d`; only array-valued content is covered and broad #612 work-operation acceptance remains open |
| Binary work comment content | Frozen Python `_work_comment` calls `.strip()` on MessagePack binary content, accepts it, and persists the trimmed bytes in the comment record. `work_comment_binary_content_matches_pinned_python_and_persists_bytes` submits the same whitespace-padded byte string to both production handlers, compares status and response body, and checks the exact trimmed binary comment content persisted on each side. Rust now preserves binary content and Python-compatible ASCII trimming instead of treating it as an empty string. | Focused local pinned-Python production-handler differential verified against Reticulum `99de23c040d507e3fefca19e87b182302902725d`; the Verify workflow runs this differential; other comment types, errors, and broad #612 operation acceptance remain open |
| Missing work comment content | An authorized production `comment` request omits `content`; both handlers return `INVALID_REQ` / `Content is required` and leave the seeded root unchanged without creating a comment file. `work_comment_missing_content_matches_pinned_python_without_mutation` compares status, exact response text, and persisted files against Reticulum `99de23c040d507e3fefca19e87b182302902725d`. | Focused local pinned-Python production-handler differential passed; the Verify workflow now runs this exact ignored test; only the absent-content request shape is covered and the broad #612 work-operation acceptance remains open |
...
| Work view float document ID | The focused ignored production-handler differential seeds documents 0 and 7 in both implementations and compares status and exact MessagePack response bodies for MessagePack float IDs `7.9` and `-0.1`. Pinned Python's `int()` truncates these examples to 7 and 0 respectively; Rust now accepts finite float IDs greater than -1 and below its existing upper bound, matching these examples. This does not establish behavior for other negative floats, non-finite values, boundary magnitudes, or other numeric types. | two pinned-Python request-shape examples; remaining numeric coercion edges and broader operation/authorization matrix remain unverified |
| Work view malformed document ID | An authorized production `view` request with string `doc_id: "not-an-id"` is passed through pinned Python's `handle_work` and Rust's `handle_work_request`. Python's integer coercion returns `INVALID_REQ / Invalid request`; Rust now returns the same status and body rather than `Invalid document request`. The existing finite-float coercion cases remain in the same differential. | one malformed string request-shape differential; other malformed types, authorization precedence, and broad #612 operation acceptance remain open |
...
| Work activation from proposed scope | The ignored `activating_proposed_work_matches_pinned_python_response_and_directory_transition` differential invokes pinned RNS 1.5.4 `ReticulumGitNode.handle_work` and Rust `handle_work_request` with an authorized proposed item. Both return status `RES_OK` and exact body bytes for MessagePack `{id: 9, scope: "active"}`; the item moves from `proposed/9` to `active/9`. Permission resolvers are stubbed to isolate the transition. | focused local pinned-Python production-handler differential against Reticulum `99de23c040d507e3fefca19e87b182302902725d`; other activation authorization/error cases and broad #612 acceptance remain open |
| Work propose success | `work_propose_success_matches_pinned_python_response_and_owner_state` submits the same valid content signature, title, format, and authenticated public identity to pinned Python 1.5.4 `handle_work` and Rust `handle_work_request_with_peer_identity`. It compares status and exact MessagePack response bytes, then verifies stored content/title/format/author/signature/public-key metadata and proposed-only placement. The writers use long (`interact`/`write`) versus short (`i`/`w`) permission aliases; both pinned Python and Rust parsers load either sidecar into identical owner permissions. | focused local pinned-Python production-handler differential; other propose denial/error cases and the broad #612 operation matrix remain open |
| Work create success | The ignored `work_create_success_matches_pinned_python_response_and_document_state` submits the same valid signed create request and authenticated public identity to pinned Python 1.5.4 `handle_work` and Rust `handle_work_request_with_peer_identity`. It compares exact status/body bytes and persisted content, title, format, author, signature, identity, and active-only placement. Python uses floating wall-clock timestamps while Rust persists integer seconds, so timestamp values are compared within one second rather than byte-for-byte. This covers one valid create-success case only. | focused local pinned-Python production-handler differential; create validation/denial/error cases and the broad #612 operation matrix remain open |
| Work create/propose missing-signature validation order | `work_create_and_propose_missing_signature_errors_match_pinned_python_precedence` sends empty title/content and no signature through pinned Python 1.5.4 `handle_work` and Rust `handle_work_request_with_peer_identity` for both `create` and `propose`. Python checks the required signature first; Rust now returns the same `INVALID_REQ` / `No signature provided` before either handler can create work storage. The test checks exact status/body and unchanged filesystem state. | focused local pinned-Python production-handler differential; other create/propose validation precedence and the broad #612 operation matrix remain open |
| Work list success | `work_list_success_matches_pinned_python_scopes_order_metadata_and_comment_counts` submits `scope=all` to both production handlers over matching persisted active/completed/proposed documents, distinct creation timestamps, metadata, and multiple numeric comment files. It compares exact status and MessagePack response bytes, covering scope grouping, descending creation order, author/format fields, and comment counts for this deterministic success fixture. | focused local pinned-Python production-handler differential verified against Reticulum `99de23c040d507e3fefca19e87b182302902725d`; other list scopes, malformed/denied paths, and the broad #612 matrix remain open |
| Work list filtered scope | `work_list_completed_scope_matches_pinned_python_and_is_read_only` submits `scope=completed` with active and completed fixtures to both production handlers. It compares exact status and MessagePack response bytes, asserts only the completed item is returned, and checks the fixture directory entries and root bytes are unchanged. The ignored success and filtered-scope tests run in the Verify workflow's pinned Python rngit step. | focused pinned-Python handler differential; other list scopes, denied/malformed paths, and the broad #612 matrix remain open |
...
The binary-comment regression was first run against the unchanged PR head and
failed as expected: Rust returned `RES_INVALID_REQ` (2), while pinned Python
returned `RES_OK` (0). After preserving and trimming the binary value, the
differential passed, including exact response comparison and persisted trimmed
bytes on both sides:

```text
RETICULUM_PY_REPO=/home/pgiuseppe/Documents/LXMF-rs/.tmp/python-refs/Reticulum-99de23c LXMF_PYTHON_BIN=python3 cargo test -p rns-tools --bin rngit tests::work_comment_binary_content_matches_pinned_python_and_persists_bytes -- --ignored --exact --nocapture --test-threads=1 PASS
RETICULUM_PY_REPO=/home/pgiuseppe/Documents/LXMF-rs/.tmp/python-refs/Reticulum-99de23c LXMF_PYTHON_BIN=python3 cargo test -p rns-tools --bin rngit tests::work_comment_malformed_content_matches_pinned_python_without_mutation -- --ignored --exact --nocapture --test-threads=1 PASS
```

The additional view request-shape differential ran in the isolated worktree at
PR #639's exact head. It exposed that pinned Python truncates float `doc_id`
through `int()` while Rust previously failed document lookup. Rust now uses the
same finite, nonnegative float-to-integer coercion. The test seeds ID 7 in both
implementations and compares status and exact MessagePack response body:

```text
RETICULUM_PY_REPO=/tmp/lxmf-606-parity-refs.hv0vPX/Reticulum-target-99de23c0 LXMF_PYTHON_BIN=python3 cargo test -p rns-tools --bin rngit --all-features work_view_float_document_id_matches_pinned_python_integer_coercion -- --ignored --nocapture --test-threads=1 PASS
```

The permission runs above verify the existing focused permission evidence and
did not expose a new permission mismatch. The broad group/repository/document
permission criterion remains open because these cases do not exhaust the
...
The pinned `handle_work` dispatcher requires repository read and, for a
document-scoped request, document read or repository admin. It then separately
requires repository admin for the `perms` operation. `_work_perms` applies the
repository read and write/interact gates before dispatching `get` or `set`, so
the author/document-admin checks inside `_work_get_permissions` do not bypass
those earlier repository gates. Rust previously allowed an author with
document-level permissions to reach `get` without the same repository-admin
gate. The production-handler differential now covers an author without
repository admin, an administrator without repository write/interact, and the
existing repository-admin fallback for a document with `read:none`. Rust now
enforces the same repository gates before looking up the document.

```text
TMPDIR=/dev/shm RETICULUM_PY_REPO=/home/pgiuseppe/Documents/LXMF-rs-issue-605/.tmp/python-refs/Reticulum \
  cargo test -p rns-tools --bin rngit --all-features permissions_get \
  -- --ignored --nocapture --test-threads=1 PASS (4 production-handler differentials)
cargo fmt --all -- --check PASS
TMPDIR=/dev/shm cargo clippy -p rns-tools --all-targets --all-features --no-deps -- -D warnings PASS
tools/scripts/check-module-size.sh PASS
git diff --check PASS
```

This closes only the tested `perms/get` authorization seam; other permission
combinations, `perms/set` ordering, network evidence, and the broader #612
