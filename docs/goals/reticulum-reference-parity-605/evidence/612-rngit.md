...
| Work view float document ID | An authorized `view` request with MessagePack `doc_id: 7.9` is coerced by pinned Python's `int()` to document 7 and returns that document. Rust now applies the same finite, nonnegative float-to-integer coercion in document-ID parsing; the focused ignored production-handler differential compares status and exact MessagePack response body using equivalent seeded documents. | one pinned-Python request-shape case; other numeric coercions and broader operation/authorization matrix remain unverified |
