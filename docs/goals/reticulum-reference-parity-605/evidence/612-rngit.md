...
| Work delete without permission sidecar | Frozen Python `_work_delete` unconditionally unlinks `<work-root>/<id>.allowed` and maps a missing file to `REMOTE_FAIL` / `Remote error` before removing the document. Rust now preserves that ordering and response instead of treating a missing sidecar as success. A regression submits delete through production `handle_request` and verifies the document remains untouched. | Local production-handler regression and pinned-source comparison; no live Python runtime differential for this edge; broader work-operation matrix remains open |
