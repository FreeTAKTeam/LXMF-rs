...
| WebP conversion | Backend preference and `RNGIT_MEDIA_BACKEND`, argv-only process construction, quality/max-dimension options, 8-second pipeline bound, bounded stderr and converted-output reads (32 MiB media-response cap), output validation, temporary-directory cleanup, raw fallback | Existing local and pinned-Python evidence covers `ffmpeg`; Verify now installs ImageMagick and GraphicsMagick and runs the production-Link conversion regression with `RNGIT_MEDIA_BACKEND=convert` and `gm`, requiring each executable (no skip), checking actual encoder argv for quality 37 and a 1x1 resize bound, validating WebP via `ffprobe` (8x4 source reduced to 1x1), filename metadata, raw fallback, and Link-scoped cleanup. This CI path is newly added and awaits a hosted run; local execution was not performed because those packages are absent and this host was not modified. `magick` (ImageMagick 7 CLI) and `avconv` remain unprovisioned/unverified; visual/reference rendering parity remains open. |
