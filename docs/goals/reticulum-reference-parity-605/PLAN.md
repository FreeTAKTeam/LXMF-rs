# Full Reticulum Reference Parity and Operational Acceptance Plan

**Intent:** Make LXMF-rs an operationally substitutable Rust implementation of the frozen Reticulum reference behavior described by issue #605, with implementation, runtime integration, differential evidence, release gates, and a finite operational support matrix that never overstates what has been verified.
...
**Expected Outcome:** The exact Python Reticulum 1.5.4 development revision named by #605 (`99de23c040d507e3fefca19e87b182302902725d`) or an explicitly reviewed replacement is frozen as the acceptance target. Every applicable reference behavior has a traceable requirement, Rust owner, executable test, and evidence record; production paths and utilities are exercised; software and operational acceptance remain separate; and the final release gate evaluates the exact candidate commit rather than historical or callable-only evidence.
**Target-Perspective Output:** A maintainer can inspect the frozen-reference manifest, behavioral inventory, delta ledger, linked implementation PRs, raw evidence, generated status, and exact-head CI/release reports and determine which rows are complete, partial, not-applicable, hardware-unverified, or blocked. An operator can inspect the finite support matrix and run the declared software/physical workflows without being told that a mock, parser, local attached-node test, or old release artifact proves a broader capability.
...
**Cutover:** Freeze the reference and behavioral contract first (#606). Demote the callable-only completion claim and fixed-count gate to historical context, while preserving old release artifacts. Promote current parity/status/release claims only after #607–#614 implementation evidence and #615 software acceptance pass; #616 remains a separate operational gate. No child PR uses `Closes #605`.
...
**Value Density:** The work is split into dependency-aware child PRs because the issue explicitly separates reference governance, software implementation, differential release acceptance, and physical/platform validation. Each child must have a disjoint owner boundary and evidence contract; the parent remains open until all required software and operational rows are proven.
**Acceptance Evidence:** Exact-source checkout and pin verification; behavioral inventory generation and deliberate-failure tests; focused Rust regressions; Python↔Rust mixed-peer and multi-process evidence; production daemon/RPC/CLI traces; exact candidate CI/release gates; and a finite platform/device/client matrix with checksummed raw reports. Passing local unit tests or finding a matching symbol is insufficient.
**Evidence Lane:** Map and freeze the reference first; add failing focused tests; implement the smallest owner-local behavior; validate deterministic/unit and simulated paths; run pinned-Python and multi-process interop; run software release gates; then collect physical/platform/client evidence. Record unavailable hardware, runners, credentials, or external peers as blocked/unverified, never as pass.
...
- Native interfaces: `crates/libs/rns-transport/src/iface`, daemon adapters, platform/feature tests, and existing HIL/prepared-host scripts (#614 and #616).
- Release/evidence gates: existing `xtask`, `tools/scripts`, `.github/workflows`, `tools/interop`, `crates/libs/test-support`, and evidence manifests (#615/#616).
...
Integration points: interface ingress/egress and IFAC, transport routing/discovery/links, Resource and Channel paths, daemon configuration/restart, RPC status, utility request/response and file side effects, rngit authorization/storage/wire schemas, native interface lifecycles, Python mixed peers, and HIL/support reports.

Migration/cutover: freeze the new target and row schema without rewriting old release claims; classify known gaps as partial/blocked/hardware-unverified; land implementation children with linked evidence; then atomically promote generated status and software release documentation for the exact candidate. Physical/support claims remain separate until #616.

Acceptance evidence gate: no parent-closure claim until #606’s deliberate-failure gates, #607–#614’s linked implementation evidence, #615’s exact-candidate software release gate, and #616’s declared support matrix are all independently reviewed and green or explicitly marked outside the software gate.
...
- Verification: hosted/native platform combinations, loopback/fake-SAM/PTY deterministic faults, and separate #616 physical evidence.
...
### 11. #616 — Physical carriers, platforms, clients, and network soak (P1, separate gate)

- Output: finite support matrix and checksummed raw evidence for named devices, platforms, external clients, network topology/load/recovery, and authorized soak criteria; unavailable rows remain blocked/unverified.
- Verification: paired RNode serial/TCP/BLE, RNodeMulti/vport/Weave, Linux/Windows/macOS/mobile rows, real I2P/network scenarios, and named client workflows with exact versions and commits.
