...
max_mesh_failures="${SCORECARD_MAX_MESH_FAILURES:-0}"
...
overall_status="PASS"
if [ "$soak_status" != "pass" ] || [ "$soak_failures" -gt "$max_soak_failures" ] || [ "$soak_mesh_failures" -gt "$max_mesh_failures" ]; then
  overall_status="FAIL"
fi
...
cat > "$json_path" <<EOF
{
  ...
  "bench_report_present": $bench_report_present,
...
| Performance budget status (advisory) | \`$perf_status\` |
| Soak status | \`$soak_status\` |
| Soak failures | \`$soak_failures\` |
| Mesh failures | \`$soak_mesh_failures\` |
| Max soak failures | \`$max_soak_failures\` |
| Max mesh failures | \`$max_mesh_failures\` |
...
- Bench budget report: \`$bench_report\` (optional advisory input)
...
\`performance_status\` (advisory), \`soak_failures\`, and \`security_pass_rows\` across releases.
EOF
